...
"@
    ...
    # --- Dashboard HTML ---
    # --- Build radar data and domain list for dashboard ---
    $radarScores = @()
    $radarLabels = @()
    $domainListHtml = ""
    foreach ($d in $sortedDomains) {
...
"@
    ...
    foreach ($cat in ($allCats.Keys | Sort-Object)) {
        ...
        foreach ($k in $keysInCat) {
            ...
            $menuItems += "<span class='nav-badge'>$short</span> $name<span class='nav-dot' data-dot='$anchor'></span></a></li>`n"
        }
        $menuItems += "  </ul>`n"
        $menuItems += "</li>`n"
    }
...
"@
    ...
    # ---------------------------------------------------------------------------
    $logCatOrder = @("System","Processes","Network","Security","Firewall","Persistence","Forensics","Logs","Vulnerabilities")
    $logsCardsHtml = ""
...
<div id="header">
  ...
  <nav class="header-nav">
    ...
    <a class="hn-link hn-logs"      href="#logs-section"         data-anchor="logs-section">Logs</a>
...
<div id="sidebar">
  ...
  <nav>
    ...
    <li><a class='nav-link nav-logs' href='#logs-section' data-anchor='logs-section'><span class='nav-badge nb-logs'>LOGS</span> Audit Logs</a></li>
$menuItems
    </ul>
  </nav>
...
(function() {
  ...
  (function() {
    ...
    var stored = localStorage.getItem('poweraudit-theme');
    ...
    function applyTheme(theme) {
      ...
      localStorage.setItem('poweraudit-theme', theme);
    }
  ...
  function setLogDot(anchor, isOpen) {
    ...
    if (!dot) return;
    if (isOpen) { dot.classList.add('dot-open'); dot.classList.remove('dot-seen'); }
    else { dot.classList.remove('dot-open'); dot.classList.add('dot-seen'); }
  }
  ...
  document.querySelectorAll('.log-cat-header[data-logcat]').forEach(function(hdr) {
    hdr.addEventListener('click', function() {
      ...
      if (grp.classList.contains('open')) closeLogCat(catId); else openLogCat(catId);
    });
  });
  document.querySelectorAll('.log-card-header[data-logid]').forEach(function(hdr) {
    hdr.addEventListener('click', function() {
      ...
      setLogDot(anchor.replace('lc-',''), isOpen);
    });
  });
  // Log card footer: colour headers + link to remediation
  (function() {
    var PRIO_COLORS={p1:{border:'#f85149',name:'#f85149'},p2:{border:'#d29922',name:'#d29922'},p3:{border:'#58a6ff',name:'#58a6ff'}};
    document.querySelectorAll('.log-footer-check').forEach(function(div) {
      var cat=div.getAttribute('data-remcat'); if (!cat) return;
      var matches=[];
      document.querySelectorAll('.rem-card').forEach(function(card) {
        var catEl=card.querySelector('.rem-cat');
        if (catEl&&catEl.textContent.trim().toLowerCase()===cat.toLowerCase()) matches.push(card);
      });
      ...
      if (matches.length>0) {
        ...
        div.innerHTML='<div class="log-footer-rem" onclick="(function(){var first=null;document.querySelectorAll(\'.rem-card\').forEach(function(c){if(first)return;var e=c.querySelector(\'.rem-cat\');if(e&&e.textContent.trim().toLowerCase()===\''+catLow+'\')first=c;});if(first){var hEl=document.getElementById(\'header\');var hh=hEl?hEl.offsetHeight:58;var top=first.getBoundingClientRect().top+window.scrollY-hh-12;window.scrollTo({top:top,behavior:\'smooth\'});setTimeout(function(){document.querySelectorAll(\'.rem-card\').forEach(function(c){var e=c.querySelector(\'.rem-cat\');if(e&&e.textContent.trim().toLowerCase()===\''+catLow+'\'){c.style.outline=\'2px solid var(--accent)\';c.style.outlineOffset=\'2px\';setTimeout(function(){c.style.outline=\'\';c.style.outlineOffset=\'\';},2500);}});},400);}})()">'+
          '<span class="lf-icon">&#9888;</span>'+
          '<span class="lf-text"><strong>'+matches.length+' remediation action'+(matches.length>1?'s':'')+' — '+cat+'</strong>'+parts.join(' &nbsp;')+'</span>'+
          '<span class="lf-arrow">&#8594; View in Remediation Plan</span></div>';
      } else {
        div.innerHTML='<div class="log-footer-ok"><span class="lf-icon">&#10003;</span><span class="lf-text"><strong>No automated finding</strong>Manual review still recommended.</span></div>';
      }
    });
  })();
