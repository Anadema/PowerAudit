########################################################################
#                                                                      #
#        PowerAudit_3.0_FR.ps1 / Releve de configuration Windows          #
#                                                                      #
#        Licence : Apache 2                                            #
...
if (-not $isAdmin) {
    ...
    Write-Host "  Relancez PowerShell avec les droits admin et reexecutez le script." -ForegroundColor Yellow
    Write-Host ""
...
# Forcer l'encodage UTF-8 pour la capture des commandes systeme (netsh, ipconfig, etc.)
# Sans cela, les caracteres accentues francais sont corrompus (CP850/CP1252 -> UTF-8)
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$OutputEncoding             = [System.Text.Encoding]::UTF8
# Forcer aussi le code page de la console Windows en UTF-8
...
function Get-AntiVirusProduct {
    try {
        ...
        [PSCustomObject]@{
            "Name"                        = "Inconnu (erreur WMI)"
            "ProductExecutable"           = ""
            "Definition Status"           = "Inconnu"
            "Real-time Protection Status" = "Inconnu"
        }
    }
}

# ---------------------------------------------------------------------------
#  CATALOGUE DES MODULES D'AUDIT
# ---------------------------------------------------------------------------

$Global:AuditModules = [ordered]@{

    "01" = @{
        Name       = "Gestionnaire de demarrage"
        ShortName  = "BCD"
        Category   = "Systeme"
        Conseil    = "[CRITIQUE] Verifier l'absence d'entrees de boot inconnues ou non signees (dual-boot suspect, boot-kit). Un boot-kit (ex: MBR root-kit) peut persister avant le chargement de Windows et echapper aux AV. Verifier que SecureBoot est actif (bcdedit /enum firmware). Toute entree avec un chemin inhabituel vers winload.exe est suspecte. [MENACE: Boot-kits, persistence pre-OS, UEFI root-kits]"
        HtmlAnchor = "bcd"
        Script     = {
            Write-SectionHeader "GESTIONNAIRE DE DEMARRAGE (bcdedit)"
            try {
    ...
    "02" = @{
        Name       = "Informations OS"
        ShortName  = "OS"
        Category   = "Systeme"
        Conseil    = "[INFO] Verifier la version et le build Windows. Un systeme non a jour est expose aux CVE publiques (ex: Eternal-Blue MS17-010 sur Win7/2008 non patche). Verifier que le poste n'est pas en fin de support (EOL) : Windows 7/8/2008 ne recoivent plus de patches. Un uptime tres long (> 30j) peut indiquer des MAJ en attente ou un redemarrage evite. [MENACE: Code-mise-en-oeuvre de vulnerabilites connues, ransom-ware, worms]"
        HtmlAnchor = "infos"
        Script     = {
            Write-SectionHeader "INFORMATIONS SYSTEME"
            ...
            $items = [ordered]@{
                "Nom de la machine"      = $cs.Name
                "Domaine / Workgroup"    = $domaine
                "Systeme d'exploitation" = $os.Caption
                "Version"                = $os.Version
    ...
    "03" = @{
        ...
        ShortName  = "ENV"
        Category   = "Systeme"
        Conseil    = "[INFO] Verifier les variables d'environnement pour detecter des chemins suspects dans PATH (ex: un dossier utilisateur avant System32 permet du DLL hijacking). Verifier TEMP/TMP : des logiciel-suspects ecrivent et executent depuis ces dossiers. Une variable COMSPEC ou PATHEXT modifiee peut indiquer une persistence ou une compromission. [MENACE: DLL hijacking, execution depuis TEMP, variables detournees]"
        HtmlAnchor = "infenv"
        Script     = {
            Write-SectionHeader "VARIABLES D'ENVIRONNEMENT"
    ...
    "04" = @{
        ...
        ShortName  = "PROC-TREE"
        Category   = "Processus"
        Conseil    = "[IMPORTANT] Analyser la hierarchie parent/enfant des processus. Anomalies typiques : cmd.exe ou powershell.exe enfant de word.exe/excel.exe (macro suspecte), svchost.exe lance par un processus autre que services.exe, explorer.exe avec des enfants inattendus. Un processus systeme (lsass, winlogon) avec un parent inhabituel indique une injection. [MENACE: Macros Office, process hollowing, injection de code, RAT]"
        HtmlAnchor = "arbps"
        Script     = {
            Write-SectionHeader "ARBRE DES PROCESSUS"
    ...
    "05" = @{
        ...
        ShortName  = "PROC-LIST"
        Category   = "Processus"
        Conseil    = "[IMPORTANT] Verifier les processus avec une consommation CPU/RAM anormale (crypto-mineur). Rechercher des noms imitant des processus systeme legitimes (svchost32.exe, lsass_.exe, svch0st.exe). Un processus sans chemin (Path vide) peut indiquer du code injecte en memoire. Verifier les processus en provenance de %TEMP%, %APPDATA% ou %PUBLIC%. [MENACE: Crypto-mineurs, RAT, process injection, living-off-the-land]"
        HtmlAnchor = "lstps"
        Script     = {
            Write-SectionHeader "LISTE DES PROCESSUS (Top 50 CPU)"
    ...
    "06" = @{
        ...
        ShortName  = "EXEC-POL"
        Category   = "Securite"
        Conseil    = "[CRITIQUE] Bypass et Unrestricted permettent l'execution de tout script PS sans restriction. La politique recommandee est RemoteSigned ou AllSigned. Un attaquant peut contourner la politique avec powershell.exe -ExecutionPolicy Bypass -File logiciel-suspect.ps1 : cette politique n'est pas une barriere de securite suffisante seule, mais une valeur permissive indique une mauvaise hygiene ou une compromission passee. [MENACE: Execution de scripts suspects, sans-fichier logiciel-suspect, PS-Empire/Invoke-Mimi-katz]"
        HtmlAnchor = "polpws"
        Script     = {
            Write-SectionHeader "POLITIQUE D'EXECUTION POWERSHELL"
    ...
    "07" = @{
        ...
        ShortName  = "SECEDIT"
        Category   = "Securite"
        Conseil    = "[CRITIQUE] Verifier : MinimumPasswordLength >= 12, PasswordComplexity = 1, MaximumPasswordAge <= 90j, LockoutBadCount <= 5 (protection brute-force), LockoutDuration >= 15 min. Un PasswordComplexity = 0 ou LockoutBadCount = 0 (pas de verrouillage) expose au brute-force local. Verifier que l'audit est active (AuditLogonEvents, AuditObjectAccess) pour la forensique. [MENACE: Brute-force local, pass-the-hash, absence de traces forensiques]"
        HtmlAnchor = "polsec"
        Script     = {
    ...
    "08" = @{
        ...
        ShortName  = "USERS"
        Category   = "Comptes"
        Conseil    = "[CRITIQUE] Le compte Administrateur integre doit etre renomme et desactive si inutilise : il est cible par defaut par les outils d'attaque (Mimi-katz, CME-tool). Le compte Invite doit etre desactive. Verifier les comptes sans mot de passe (PasswordRequired=False) : vecteur trivial d'acces local. Verifier PasswordNeverExpires=True sur des comptes actifs (mauvaise pratique). Tout compte inconnu ou inactif depuis > 90j doit etre desactive. [MENACE: Brute-force, mouvement lateral, escalade de privileges]"
        HtmlAnchor = "licpt"
        Script     = {
            Write-SectionHeader "COMPTES LOCAUX"
    ...
    "09" = @{
        ...
        ShortName  = "GROUPS"
        Category   = "Comptes"
        Conseil    = "[IMPORTANT] Le groupe Administrateurs ne doit contenir que les comptes strictement necessaires. Verifier l'absence de comptes de service ou generiques dans les groupes privilegies. Le groupe Remote Desktop Users contr le acces RDP : limiter au strict necessaire. Des membres inconnus dans Administrateurs ou Operateurs de sauvegarde indiquent une compromission. [MENACE: Escalade de privileges, persistence via groupe, mouvement lateral RDP]"
        HtmlAnchor = "ligrp"
        Script     = {
            Write-SectionHeader "GROUPES LOCAUX"
            $output = @()
    ...
    "10" = @{
        Name       = "Partages reseau"
        ShortName  = "SHARES"
        Category   = "Reseau"
        Conseil    = "[CRITIQUE] Les partages administratifs (C$, D$, ADMIN$, IPC$) sont des vecteurs de mouvement lateral code-utilises par les ransom-wares (Wanna-Cry, Not-Petya) et les outils comme Ps-Exec/CME-tool. Desactiver avec : reg add HKLM\System\CurrentControlSet\Services\LanmanServer\Parameters /v AutoShareWks /t REG_DWORD /d 0. Verifier que SYSVOL/NETLOGON ne sont exposes que si le poste est controleur de domaine. Tout partage non documente est suspect. [MENACE: Ransom-ware, mouvement lateral SMB, exfiltration de donnees]"
        HtmlAnchor = "shares"
        Script     = {
            Write-SectionHeader "PARTAGES RESEAU"
    ...
    "11" = @{
        ...
        ShortName  = "USB"
        Category   = "Systeme"
        Conseil    = "[IMPORTANT] L'historique USB revele tous les peripheriques connectes depuis l'installation. Verifier la presence de cles USB inconnues, d'adaptateurs reseau USB (exfiltration), de rubber ducky / bad USB (emulation clavier HID pour injection de commandes). En environnement sensible, la politique de groupe doit bloquer les peripheriques USB non autorises. Chaque entree inconnue doit etre justifiee par l'utilisateur. [MENACE: Bad-USB, exfiltration physique, introduction de logiciel-suspect, HID injection]"
        HtmlAnchor = "usb"
        Script     = {
            Write-SectionHeader "HISTORIQUE DES SUPPORTS USB"
            $result = Get-USBKeys
            if ($result) {
                $result | ForEach-Object {
                ...
                Write-Host "  Aucun peripherique USB trouve dans le registre." -ForegroundColor DarkGray
    ...
    "12" = @{
        ...
        ShortName  = "DISK"
        Category   = "Systeme"
        Conseil    = "[INFO] Un espace disque inferieur a 10%% peut indiquer une collecte massive de logs/donnees par un logiciel-suspect ou une exfiltration en cours. Verifier la presence de partitions cachees ou non montees (vecteur de persistence root-kit). Un chiffrement BitLocker doit etre actif sur les postes mobiles (verifier avec manage-bde -status). [MENACE: Exfiltration, ransom-ware pre-chiffrement, absence de chiffrement disque]"
        HtmlAnchor = "part"
        Script     = {
            Write-SectionHeader "PARTITIONS ET DISQUES"
    ...
    "13" = @{
        Name       = "Configuration reseau"
        ShortName  = "IPCONFIG"
        Category   = "Reseau"
        Conseil    = "[IMPORTANT] Verifier que les DNS configures appartiennent a l'organisation ou a des fournisseurs connus. Un DNS modifie (ex: 8.8.8.8 remplace par une IP inconnue) peut indiquer un DNS hijacking. Plusieurs adresses IP sur une interface peuvent signaler un tunnel ou un proxy. Une adresse APIPA (169.254.x.x) indique un probleme DHCP pouvant isoler le poste. Verifier l'absence d'interfaces virtuelles non documentees (VPN, Tor, tunnel). [MENACE: DNS hijacking, C2 via DNS, tunnel reseau, pivot]"
        HtmlAnchor = "cfrzo"
        Script     = {
    ...
    "14" = @{
        Name       = "Connexions reseau actives"
        ShortName  = "NETSTAT"
        Category   = "Reseau"
        Conseil    = "[CRITIQUE] Verifier les connexions ESTABLISHED vers des IPs externes inconnues : possible communication C2 (Command & Control) d'un logiciel-suspect. Des ports en ecoute inhabituels (>1024 sur une machine de bureau) peuvent indiquer un RAT ou un back-door. Verifier les processus associes aux ports : un svchost.exe ecoutant sur un port non standard est suspect. Les connexions repetees vers la meme IP distante en TIME_WAIT peuvent signaler un balisage-C2. [MENACE: RAT, C2, back-door, balisage-C2, exfiltration reseau]"
        HtmlAnchor = "statrzo"
        Script     = {
    ...
    "15" = @{
        ...
        ShortName  = "ROUTE"
        Category   = "Reseau"
        Conseil    = "[IMPORTANT] Une route par defaut inattendue peut rediriger tout le trafic vers un proxy suspect. Des routes statiques ajoutees vers des sous-reseaux internes depuis un poste de travail peuvent indiquer un pivot reseau ou un implant de type agent de mouvement lateral. Verifier que la passerelle par defaut correspond bien a l'infrastructure connue. [MENACE: Pivot reseau, man-in-the-middle, exfiltration via route forcee]"
        HtmlAnchor = "route"
        Script     = {
            Write-SectionHeader "TABLE DE ROUTAGE"
    ...
    "16" = @{
        Name       = "Source mises a jour (WSUS)"
        ShortName  = "WSUS"
        Category   = "MAJ"
        Conseil    = "[IMPORTANT] Si un serveur WSUS est configure, verifier qu'il s'agit bien d'un serveur interne connu. Un WSUS detourne (attaque WSUS-pect) permet a un attaquant de distribuer de fausses mises a jour signees a tous les postes du domaine, conduisant a une compromission totale du parc. Si aucun WSUS n'est configure, verifier que les MAJ automatiques Windows Update sont actives. [MENACE: WSUS-pect, fausses MAJ, compromission du parc entier via WSUS]"
        HtmlAnchor = "wsus"
        Script     = {
            Write-SectionHeader "SOURCE DES MISES A JOUR (WSUS)"
            ...
            if (($result -join "") -match "ERROR|Erreur") {
                Write-Host "  Aucune configuration WSUS trouvee (mise a jour directe Microsoft)." -ForegroundColor Yellow
            } else {
                $result | ForEach-Object { Write-Host "  $_" -ForegroundColor Cyan }
            }
            return ($result -join "`n")
    ...
    "17" = @{
        Name       = "Mises a jour installees"
        ShortName  = "HOTFIX"
        Category   = "MAJ"
        Conseil    = "[CRITIQUE] Verifier la date de la derniere mise a jour : plus de 30j sans patch est risque, plus de 90j est critique. Rechercher les CVE critiques non patchees : Eternal-Blue (MS17-010), Print-Nightmare (CVE-2021-34527), Log4-Shell, CVE-2022-30190 (CVE-2022-30190). Comparer le build Windows avec les releases Microsoft pour identifier les patches manquants. L'absence de certains KB de securite est un indicateur fort de surface d'attaque ouverte. [MENACE: Code-mise-en-oeuvre de CVE connues, ransom-ware, worm de propagation]"
        HtmlAnchor = "maj"
        Script     = {
            Write-SectionHeader "MISES A JOUR INSTALLEES"
    ...
    "18" = @{
        ...
        ShortName  = "NTP"
        Category   = "Reseau"
        Conseil    = "[INFO] Un decalage horaire important (drift > 5 min) peut indiquer une manipulation du temps pour fausser les logs et compliquer l'analyse forensique. Un serveur NTP externe non autorise peut etre signe d'une reconfiguration suspecte. Dans un domaine AD, tous les postes doivent se synchroniser sur le DC (W32TM /query /source). La coherence du temps est critique pour la correlation des evenements dans un SIEM. [MENACE: Anti-forensique, alteration des logs, invalidation des certificats Kerberos]"
        HtmlAnchor = "ntp"
        Script     = {
    ...
    "19" = @{
        ...
        ShortName  = "WIFI"
        Category   = "Reseau"
        Conseil    = "[IMPORTANT] Verifier que le poste ne se connecte pas automatiquement a des reseaux Wi-Fi ouverts (evil twin, rogue AP). Les profils Wi-Fi stockes contiennent les mots de passe en clair recuperables via : netsh wlan show profile <nom> key=clear. Un adaptateur Wi-Fi en mode Monitor ou AP peut indiquer un outil de scan/pivot reseau. Desactiver le Wi-Fi sur les postes fixes en environnement sensible. [MENACE: Evil twin, vol de credentials Wi-Fi, rogue AP, pivot sans fil]"
        HtmlAnchor = "wifi"
        Script     = {
            ...
            $result | ForEach-Object {
                ...
                elseif ($line -match "Authentification|Chiffrement") { $color = "Cyan" }
                Write-Host "  $line" -ForegroundColor $color
            }
            return ($result -join "`n")
        }
    ...
    "20" = @{
        ...
        ShortName  = "DNS-CACHE"
        Category   = "Reseau"
        Conseil    = "[IMPORTANT] Le cache DNS revele les domaines recemment contactes par le poste. Rechercher des domaines suspects : DGA (noms aleatoires type xk3m9p2.net), domaines de C2 connus, domaines ressemblant a des marques (typosquatting : micosoft.com). Un grand nombre de requetes DNS echouees peut indiquer un logiciel-suspect DGA cherchant son serveur C2. Verifier via VirusTotal ou Shodan les IPs/domaines inconnus. [MENACE: C2 via DNS, DGA, DNS tunneling, exfiltration de donnees via DNS]"
        HtmlAnchor = "dns"
        Script     = {
            Write-SectionHeader "CACHE DNS"
            ...
            Write-Host ""
            Write-Host "  ---- DOMAINES BRUTS ($($rawDomains.Count) uniques) ----" -ForegroundColor Cyan
    ...
    "22" = @{
        ...
        ShortName  = "ARP"
        Category   = "Reseau"
        Conseil    = "[INFO] Une table ARP avec des doublons d'adresses MAC (deux IP avec le meme MAC ou inversement) peut indiquer une attaque ARP spoofing / ARP poisoning en cours sur le segment reseau. L'ARP poisoning permet le MITM : l'attaquant redirige le trafic vers lui avant de le retransmettre. Verifier que la passerelle par defaut a toujours la meme adresse MAC entre deux audits. [MENACE: ARP spoofing, man-in-the-middle, interception reseau locale]"
        HtmlAnchor = "arp"
        Script     = {
            Write-SectionHeader "TABLE ARP"
    ...
    "23" = @{
        ...
        ShortName  = "HOSTS"
        Category   = "Reseau"
        Conseil    = "[CRITIQUE] Le fichier hosts est modifie par de nombreux outils suspects pour : bloquer les mises a jour AV (rediriger windowsupdate.com vers 127.0.0.1), rediriger des sites bancaires vers des serveurs de hame-connage, empecher la communication avec des serveurs de remediation. Le fichier ne doit contenir que la ligne localhost (127.0.0.1 localhost) et eventuellement ::1 localhost. Toute autre entree doit etre justifiee. Hash a comparer entre audits. [MENACE: Evasion-AV, hame-connage local, blocage AV/MAJ, DNS hijacking local]"
        HtmlAnchor = "hosts"
        Script     = {
            Write-SectionHeader "FICHIER HOSTS"
    ...
    "24" = @{
        ...
        HtmlAnchor = "svc"
        Script     = {
            Write-SectionHeader "ETAT DES SERVICES"
    ...
    "25" = @{
        ...
        ShortName  = "FW-STATE"
        Category   = "Pare-feu"
        Conseil    = "[CRITIQUE] Le pare-feu doit etre actif sur les trois profils (Domain, Private, Public). Un pare-feu desactive sur le profil Public est une faute grave (connexions non filtrees sur reseau inconnu). Le profil Domain est souvent plus permissif : verifier que le profil Public/Private est bien plus restrictif. Un pare-feu desactive peut etre le resultat d'une compromission (logiciel-suspect desactivant la protection) ou d'une mauvaise pratique administrative. [MENACE: Connexions non filtrees, scan de ports, code-mise-en-oeuvre directe de services exposes]"
        HtmlAnchor = "conffw"
        Script     = {
    ...
    "26" = @{
        Name       = "Pare-feu - regles entrantes"
        ShortName  = "FW-IN"
        Category   = "Pare-feu"
        ...
        Script     = {
            ...
            $rules = Get-NetFirewallRule -Direction Inbound -Enabled True |
                ...
                Sort-Object Action
            Write-Host "  Total regles actives entrantes: $($rules.Count)" -ForegroundColor Yellow
    ...
    "27" = @{
        Name       = "Pare-feu - regles sortantes"
        ShortName  = "FW-OUT"
        Category   = "Pare-feu"
        Conseil    = "[INFO] Les regles sortantes sont souvent negligees mais critiques pour la defense en profondeur. Un pare-feu bloquant par defaut le trafic sortant limite considerablement les capacites d'un logiciel-suspect : connexion au C2, telechargement de charge-utile supplementaire, exfiltration de donnees. Verifier que PowerShell, cmd.exe et mshta.exe ne sont pas autorises a initier des connexions sortantes vers Internet (indicateur de sans-fichier logiciel-suspect ou LOLBin). [MENACE: C2, exfiltration, LOLBins (mshta, certutil, bitsadmin), sans-fichier logiciel-suspect]"
        HtmlAnchor = "outfw"
        Script     = {
            ...
            $rules = Get-NetFirewallRule -Direction Outbound -Enabled True |
                Select-Object DisplayName, Action, Protocol, Profile |
                Sort-Object Action
            Write-Host "  Total regles actives sortantes: $($rules.Count)" -ForegroundColor Yellow
            $rules | ForEach-Object {
                $color = if ($_.Action -eq "Allow") { "Green" } else { "Red" }
                Write-Host ("  [{0,-6}] {1}" -f $_.Action, $_.DisplayName) -ForegroundColor $color
            }
    ...
    "28" = @{
        ...
        ShortName  = "AV"
        Category   = "Securite"
        Conseil    = "[CRITIQUE] L'antivirus doit etre actif (Real-time Protection = Enabled) et a jour (Up to date). Des signatures vieilles de plus de 7 jours n'offrent plus de protection efficace contre les menaces recentes. Verifier que l'AV n'a pas ete desactive : certains ransom-wares (RE-vil, Con-ti) desactivent l'AV avant de commencer le chiffrement. La presence de plusieurs AV simultanement peut creer des conflits et affaiblir la protection. Completer avec EDR/XDR en environnement sensible. [MENACE: Ransom-ware, Contournement-AV, desactivation de protection, zero-day]"
        HtmlAnchor = "infav"
        Script     = {
    ...
    "29" = @{
        ...
        Conseil    = "[IMPORTANT] Les taches planifiees sont un vecteur de persistence tres utilise par les logiciel-suspects. Verifier les taches dans \Microsoft\Windows\... avec des actions pointant vers %TEMP%, %APPDATA%, ou executant powershell.exe / wscript.exe / cscript.exe / mshta.exe avec des arguments suspects. Verifier les taches creees recemment (date de creation proche d'un incident). Une tache sans auteur connu ou avec un GUID aleatoire comme nom est suspecte. [MENACE: Persistence logiciel-suspect, execution differee de charge-utile, porte derobee planifiee]"
        HtmlAnchor = "tasks"
        Script     = {
    ...
    "30" = @{
        Name       = "Logiciels installes"
        ShortName  = "APPS"
        Category   = "Systeme"
        Conseil    = "[IMPORTANT] Verifier la presence de logiciels non autorises : outils d'administration a distance (AnyDesk, TeamViewer, RustDesk) installes a l'insu du SI, outils de test-intrusion (Nmap, Wireshark, Msf-framework), logiciels de minage de cryptomonnaies. Verifier les versions : Java, Adobe Reader, navigateurs et Office obsoletes sont des vecteurs d'code-mise-en-oeuvre majeurs. Un logiciel installe recemment (date proche d'un incident) doit etre examine en priorite. [MENACE: RAT deguise en outil legitime, code-mise-en-oeuvre de logiciels obsoletes, crypto-mineur]"
        HtmlAnchor = "logs"
        Script     = {
            ...
            Write-Host "  Total: $($apps.Count) logiciels" -ForegroundColor Yellow
            $apps | ForEach-Object {
    ...
    "31" = @{
        Name       = "Journal Systeme (50 derniers)"
        ShortName  = "EVT-SYS"
        Category   = "Journaux"
    ...
    "32" = @{
        ...
        ShortName  = "EVT-APP"
        Category   = "Journaux"
        Conseil    = "[INFO] Les erreurs applicatives repetitives peuvent indiquer un logiciel instable (crash loop). Verifier les erreurs provenant de processus inconnus ou avec des chemins dans %TEMP%/%APPDATA%. Des erreurs CertEnroll/certificate peuvent signaler un probleme de PKI ou une tentative d'inscription de certificat suspect. Les crashs d'applications de securite (AV, EDR) peuvent etre intentionnels. [MENACE: Outil-suspect crashant des protections, injection applicative, instabilite suspecte]"
        HtmlAnchor = "evtapp"
        Script     = {
    ...
    "33" = @{
        ...
        ShortName  = "GPO"
        Category   = "Securite"
        Conseil    = "[IMPORTANT] Verifier que les GPO de securite attendues sont bien appliquees : politique de mots de passe, restriction logiciels (SRP/AppLocker), desactivation des macros Office, blocage des peripheriques amovibles, configuration du pare-feu. Des GPO non appliquees (RSOP vide) peuvent indiquer une rupture de lien AD ou une manipulation. Verifier que les GPO ne contiennent pas de scripts de demarrage ou de connexion non documentes. [MENACE: Contournement de politiques de securite, persistence via GPO, mouvement lateral AD]"
        HtmlAnchor = "rsop"
        Script     = {
    ...
    "34" = @{
        Name       = "Programmes au demarrage"
        ShortName  = "STARTUP"
        Category   = "Persistance"
        Conseil    = "[CRITIQUE] Les cles Run/RunOnce sont le vecteur de persistence le plus utilise par les logiciel-suspects. Verifier chaque entree : le chemin doit pointer vers un executable connu dans un dossier systeme, pas vers %TEMP%, %APPDATA%, %PUBLIC% ou un chemin avec des caracteres aleatoires. Une entree encodee en base64 ou avec des arguments powershell -enc est un indicateur fort de compromission. Comparer avec une baseline saine du poste. Verifier aussi : HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon (Userinit, Shell). [MENACE: Persistence logiciel-suspect, RAT, ransom-ware, sans-fichier via registre]"
        HtmlAnchor = "logdem"
        Script     = {
    ...
    "35" = @{
        Name       = "IPv6 - Adresses et Privacy"
        ShortName  = "IPv6"
        Category   = "Reseau"
        Conseil    = "[INFO] Si IPv6 n'est pas utilise dans l'organisation, le desactiver pour reduire la surface d'attaque. Les adresses IPv6 de type Teredo ou 6to4 peuvent etre utilisees pour tunneliser du trafic et contourner des pare-feux ne filtrant pas IPv6. SLAAC (adressage automatique IPv6) peut attribuer des adresses sans controle DHCP, rendant la tracabilite difficile. Un voisin IPv6 inconnu dans la table des voisins peut indiquer un scanner ou un pivot. [MENACE: Tunnel IPv6, contournement de pare-feu, scan reseau, pivot via Teredo]"
        HtmlAnchor = "adip6"
        Script     = {
            Write-SectionHeader "IPv6 - ADRESSES, INTERFACES, PRIVACY, NEIGHBORS"
            Write-Host "  -- Adresses --" -ForegroundColor Yellow
    ...
    "36" = @{
        Name       = "BitLocker - Chiffrement disque"
        ShortName  = "BITLOCKER"
        Category   = "Chiffrement"
        Conseil    = "[CRITIQUE] Tout poste mobile doit avoir BitLocker actif sur C:. Un disque non chiffre permet la lecture directe des donnees en cas de vol (boot sur USB, extraction du disque). Verifier que la cle de recuperation est sauvegardee dans AD ou Azure AD. ProtectionStatus=Off indique que le chiffrement est suspendu (typiquement apres une MAJ BIOS) : il doit etre reactive rapidement. [MENACE: Vol de donnees par extraction disque, acces physique non autorise, vol de portable]"
        ...
        Script     = {
            ...
            try {
                ...
                $vols | ForEach-Object {
                    ...
                    $line = ("  {0,-6} Protection:{1,-10} Chiffrement:{2,4}%  Methode:{3}" -f `
                ...
                Write-Host "  BitLocker non disponible ou non configure sur ce systeme." -ForegroundColor Yellow
                $output += "BitLocker non disponible."
            }
            return ($output -join "`n")
        }
    }
    ...
    "37" = @{
        Name       = "Certificats - Magasin systeme"
        ShortName  = "CERTS"
        Category   = "Chiffrement"
        Conseil    = "[IMPORTANT] Verifier la presence de certificats racine inconnus dans Cert:\LocalMachine\Root : un certificat racine malveillant permet l'intercepton HTTPS transparente (MITM sur tout le trafic TLS). Verifier les certificats expires qui pourraient bloquer des services. Les certificats auto-signes dans le magasin de confiance sont suspects en environnement d'entreprise. [MENACE: Interception TLS/HTTPS, MITM via faux certificat racine, espionnage reseau]"
        HtmlAnchor = "certs"
        Script     = {
            ...
            foreach ($storeName in $stores) {
                try {
                    ...
                    Write-Host ("  -- Magasin : LocalMachine\{0} ({1} certificats) --" -f $storeName, $certs.Count) -ForegroundColor Yellow
    ...
    "38" = @{
        Name       = "Journal Securite (50 derniers)"
        ShortName  = "EVT-SEC"
        Category   = "Forensique"
        Conseil    = "[CRITIQUE] Le journal Securite contient les evenements d'authentification critiques. ID 4625 (echec logon) en masse = brute-force en cours. ID 4648 (logon avec credentials explicites) = pass-the-hash potentiel. ID 4720/4732 (creation compte / ajout groupe) = persistence suspecte. ID 4698/4702 (creation tache planifiee) = persistence. ID 4688 (creation processus) si audit active = tracabilite des executions. [MENACE: Brute-force, pass-the-hash, creation de compte suspect, escalade de privileges]"
        HtmlAnchor = "evtsec"
        Script     = {
            ...
            try {
                ...
                $msg = "Acces refuse au journal Securite ou journal vide. Verifier les droits administrateur."
    ...
    "39" = @{
        Name       = "Fichiers recemment modifies (72h)"
        ShortName  = "RECENT-FILES"
        Category   = "Forensique"
        Conseil    = "[IMPORTANT] Des fichiers modifies recemment dans des dossiers sensibles peuvent indiquer une activite suspecte. Verifier les modifications dans : System32, dossiers de demarrage, profils utilisateurs. Des .exe ou .dll apparus recemment dans %TEMP% ou %APPDATA% sont des indicateurs forts de compromission. Des fichiers avec des extensions doubles (doc.exe, pdf.exe) sont caracteristiques des RAT. [MENACE: Depot de charge-utile, modification de binaires systeme, dropper]"
        ...
        Script     = {
            ...
            foreach ($path in $searchPaths) {
                ...
                if ($files.Count -gt 0) {
                    Write-Host ("  -- {0} ({1} fichier(s) suspect(s)) --" -f $path, $files.Count) -ForegroundColor Yellow
                    $output += ("=== $path ===")
                    ...
                    Write-Host ("  {0} : aucun fichier executable modifie recemment" -f $path) -ForegroundColor DarkGray
                }
            }
            return ($output -join "`n")
        }
        ...
        Script     = {
            ...
            if (Test-Path $rdpKey) {
                Get-ChildItem $rdpKey -ErrorAction SilentlyContinue | ForEach-Object {
                    ...
                    $line   = ("  Serveur: {0,-40}  Utilisateur: {1}" -f $server, $user)
                    Write-Host $line -ForegroundColor Cyan
                    $output += $line
                }
            } else {
                Write-Host "  Aucun historique de connexion RDP client." -ForegroundColor DarkGray
                $output += "Aucun historique RDP client."
            }
            # Connexions RDP recues (journal evenements)
            Write-Host ""
            Write-Host "  -- Connexions RDP recues (journal TerminalServices) --" -ForegroundColor Yellow
            ...
            try {
                ...
                if ($rdpEvents) {
                    ...
                    Write-Host "  Aucune connexion RDP recue recemment." -ForegroundColor DarkGray
                    $output += "Aucune connexion RDP recente."
                }
            } catch {
                Write-Host "  Journal TerminalServices inaccessible ou RDP non utilise." -ForegroundColor DarkGray
                $output += "Journal TerminalServices non disponible."
            }
            return ($output -join "`n")
        }
    }
    ...
    "41" = @{
        Name       = "Activite PowerShell (historique)"
        ShortName  = "PS-HIST"
        Category   = "Forensique"
        Conseil    = "[CRITIQUE] L'historique PowerShell contient les commandes recemment executees par l'utilisateur. Des commandes encodees (base64), des appels a DownloadString/WebClient/Invoke-Expression, des connexions reseau depuis PS (New-Object Net.WebClient) sont des indicateurs forts d'activite suspecte ou d'attaque sans-fichier. L'historique PS peut aussi etre efface par un attaquant : son absence totale est suspecte. [MENACE: Attaque sans-fichier, telechargement de charge-utile, execution distante via PS]"
        HtmlAnchor = "pshist"
        Script     = {
            Write-SectionHeader "HISTORIQUE POWERSHELL"
            $output = @()
            $histPaths = @(
            ...
            foreach ($histPath in $histPaths) {
                if (Test-Path $histPath) {
                    ...
                    Write-Host ("  Fichier : $histPath ({0} lignes totales)" -f (Get-Content $histPath).Count) -ForegroundColor Yellow
                    $output += "=== $histPath ==="
            ...
            if (-not $found) {
                Write-Host "  Historique PowerShell introuvable (PSReadLine non installe ou efface)." -ForegroundColor Yellow
                $output += "Historique PowerShell non disponible ou efface."
            }
            return ($output -join "`n")
        }
    }
    ...
    "42" = @{
        Name       = "Droits UAC et privileges"
        ShortName  = "UAC"
        Category   = "Vulnerabilites"
        Conseil    = "[CRITIQUE] UAC desactive supprime la demande de confirmation pour les operations privilegiees, permettant a tout processus d'obtenir les droits SYSTEM sans interaction. ConsentPromptBehaviorAdmin=0 signifie elevation silencieuse. Verifier que l'utilisateur courant n'est pas membre du groupe Administrateurs de facon permanente (principe du moindre privilege). [MENACE: Escalade de privileges silencieuse, contournement UAC, execution en SYSTEM sans alerte]"
        HtmlAnchor = "uac"
        Script     = {
            ...
            $uacLabels = @{
                ...
                "PromptOnSecureDesktop"      = @{0="[RISQUE] Bureau non securise"; 1="[OK] Bureau securise"}
            }
            foreach ($k in $items.Keys) {
                $val = $items[$k]
            ...
            Write-Host "  -- Privileges du token courant --" -ForegroundColor Yellow
    ...
    "43" = @{
        ...
        ShortName  = "APPLOCKER"
        Category   = "Vulnerabilites"
        Conseil    = "[IMPORTANT] AppLocker et Software Restriction Policies limitent l'execution aux executables autorises. Sans ces politiques, tout utilisateur peut executer n'importe quel binaire, script PS, VBS, ou HTA. AppLocker doit bloquer les executions depuis TEMP, APPDATA et les dossiers utilisateurs. L'absence de regles AppLocker sur un poste sensible est une lacune de securite majeure. [MENACE: Execution de charge-utile, LOLBins, scripts non autorises, contournement AV]"
        ...
        Script     = {
            ...
            try {
                ...
                $line   = ("  Service AppIDSvc (AppLocker) : {0}" -f $alSvc.Status)
                Write-Host $line -ForegroundColor $color
                $output += $line
                ...
                if (Test-Path $alKey) {
                    ...
                    foreach ($cat in $categories) {
                        ...
                        if (Test-Path $catKey) {
                            ...
                            $line = ("  Regles {0,-8}: {1} regle(s) configuree(s)" -f $cat, $rules.Count)
                            $color = if ($rules.Count -gt 0) { "Green" } else { "Yellow" }
                            Write-Host $line -ForegroundColor $color
                            $output += $line
                        } else {
                            $line = ("  Regles {0,-8}: aucune regle" -f $cat)
                            Write-Host $line -ForegroundColor DarkGray
                            $output += $line
                        }
                    }
                } else {
                    Write-Host "  AppLocker non configure (aucune politique SrpV2)." -ForegroundColor Red
                    $output += "AppLocker non configure."
                }
            } catch {
                Write-Host "  Service AppLocker non disponible sur cette edition Windows." -ForegroundColor DarkGray
                $output += "AppLocker non disponible."
            }
            ...
            if (Test-Path $srpKey) {
                Write-Host "  SRP configure." -ForegroundColor Green
                $output += "SRP configure."
            } else {
                Write-Host "  SRP non configure." -ForegroundColor Red
                $output += "SRP non configure."
            }
            return ($output -join "`n")
        }
    }
    ...
    "44" = @{
        ...
        ShortName  = "LSAPROT"
        Category   = "Vulnerabilites"
        Conseil    = "[CRITIQUE] LSA Protection (RunAsPPL) empeche la lecture de la memoire de lsass.exe par des outils de vol de credentials. Sans cette protection, Mimi-katz peut extraire les hash et mots de passe en clair depuis lsass en quelques secondes. Credential Guard isole les credentials dans un environnement virtualise. Ces deux protections sont essentielles sur tout poste d'administrateur. [MENACE: Vol de credentials depuis lsass, pass-the-hash, pass-the-ticket]"
        HtmlAnchor = "lsaprot"
        Script     = {
            ...
            if ($cgProps) {
                ...
                Write-Host "  Device Guard / Credential Guard : cle registre absente." -ForegroundColor DarkGray
                $output += "Device Guard non configure."
            }
            # WDigest (stockage mot de passe en clair)
            Write-Host ""
            Write-Host "  -- WDigest (mots de passe en clair en memoire) --" -ForegroundColor Yellow
            ...
            $wdLabel = if ($useLogonCred -eq 1) { "[CRITIQUE] WDigest actif : mots de passe en clair en memoire !" } else { "[OK] WDigest desactive (recommande)" }
    ...
    "45" = @{
        Name       = "Connexions reseau etablies - Geoip"
        ShortName  = "NET-GEO"
        Category   = "Vulnerabilites"
        ...
        Script     = {
            ...
            try {
                ...
                if ($conns) {
                    ...
                    Write-Host "  Aucune connexion etablie vers des IPs externes." -ForegroundColor DarkGray
                    $output += "Aucune connexion externe etablie."
                }
            } catch {
    ...
    "46" = @{
        Name       = "Modules PowerShell charges"
        ...
        Script     = {
            ...
            Write-Host "  -- Modules PS installes (hors systeme) --" -ForegroundColor Yellow
            $output += "=== Modules PS ==="
            ...
            if ($modules) {
                ...
                Write-Host "  Aucun module non-systeme installe." -ForegroundColor DarkGray
                $output += "Aucun module non-systeme."
            }
            # Journalisation PS
            Write-Host ""
            Write-Host "  -- Configuration journalisation PowerShell --" -ForegroundColor Yellow
            ...
            foreach ($pk in $psKeys) {
                ...
                $label = if ($val -eq 1) { "[ACTIVE]" } else { "[INACTIF] - recommande pour la detection" }
...
function Compute-SecurityScore {
    ...
    if ($Results.ContainsKey("01")) {
        $s=10; $i=@()
        if (Has "01" "(?i)bootdebug\s+yes|testsigning\s+yes") { $s-=4; $i+="Mode debug/test actif (execution de code non signe)" }
        if (Has "01" "(?i)recoveryenabled\s+no")               { $s-=2; $i+="Recuperation desactivee" }
        AddDomain "Demarrage (BCD)" "bcd" $s $i 2
    }
    ...
    if ($Results.ContainsKey("02")) {
        $s=10; $i=@()
        $r = ResultOf "02"
        if ($r -match "(?i)Windows 7|Windows 8[^.1]|2008|2003|2000|XP|Vista") { $s-=7; $i+="OS en fin de support (EOL)" }
        ...
        if ($r -match "Uptime\s*:\s*(\d+)j") {
            ...
            elseif ($d -gt 30) { $s-=1; $i+="Uptime $d jours - verifier les MAJ" }
        }
        AddDomain "Systeme d'exploitation" "infos" $s $i 3
    }
    ...
    if ($Results.ContainsKey("03")) {
        ...
        if ($r -match "(?i)PATH.*Users.*System32|PATH.*Temp") { $s-=3; $i+="PATH suspect : dossier utilisateur avant System32 (risque DLL hijacking)" }
    ...
    if ($Results.ContainsKey("05")) {
        ...
        if ($suspProc.Count -gt 0) { $s-=5; $i+="$($suspProc.Count) processus depuis TEMP/APPDATA/PUBLIC detecte(s)" }
    ...
    if ($Results.ContainsKey("06")) {
        $s=10; $i=@()
        if (Has "06" "(?i)Unrestricted") { $s-=6; $i+="ExecutionPolicy Unrestricted : tout script PS executable" }
        elseif (Has "06" "(?i)Bypass")   { $s-=5; $i+="ExecutionPolicy Bypass detecte" }
        elseif (Has "06" "(?i)Undefined") { $s-=2; $i+="ExecutionPolicy non definie sur un ou plusieurs scopes" }
    ...
    if ($Results.ContainsKey("07")) {
        ...
        if ($r -match "PasswordComplexity\s*=\s*0") { $s-=3; $i+="Complexite des mots de passe desactivee" }
        if ($r -match "LockoutBadCount\s*=\s*0")    { $s-=3; $i+="Pas de verrouillage apres echecs (brute-force possible)" }
        if ($r -match "MaximumPasswordAge\s*=\s*0")  { $s-=2; $i+="Expiration des mots de passe desactivee" }
        AddDomain "Politique de securite" "polsec" $s $i 3
    }
    ...
    if ($Results.ContainsKey("08")) {
        $s=10; $i=@()
        $r = ResultOf "08"
        if ($r -match "(?i)(Administrator|Administrateur).*Actif\s*:\s*True") { $s-=3; $i+="Compte Administrateur integre actif" }
        if ($r -match "(?i)(Guest|Invite).*Actif\s*:\s*True")                  { $s-=3; $i+="Compte Invite/Guest actif" }
        if ($r -match "(?i)\[!MOT DE PASSE VIDE\]")                            { $s-=5; $i+="Compte(s) sans mot de passe detecte(s)" }
    ...
    if ($Results.ContainsKey("09")) {
        ...
        if ($admCount -gt 3) { $s-=3; $i+="$admCount membres dans le groupe Administrateurs (principe moindre privilege)" }
        elseif ($admCount -gt 2) { $s-=1; $i+="$admCount membres dans Administrateurs - verifier si tous necessaires" }
    ...
    if ($Results.ContainsKey("10")) {
        ...
        if ($totalLines -gt 8) { $s-=2; $i+="Nombreux partages detectes - verifier les non documentes" }
        AddDomain "Partages reseau (SMB)" "shares" $s $i 3
    }
    ...
    if ($Results.ContainsKey("11")) {
        ...
        if ($usbCount -gt 10) { $s-=3; $i+="$usbCount peripheriques USB historiques - verifier les inconnus" }
    ...
    if ($Results.ContainsKey("12")) {
        ...
        ($r -split "`n") | ForEach-Object {
            if ($_ -match "\((\d+)%\)") {
                $pct = [int]$Matches[1]
                if ($pct -lt 10) { $s-=3; $i+="Partition avec seulement $pct`% d'espace libre" }
                elseif ($pct -lt 20) { $s-=1; $i+="Partition avec $pct`% d'espace libre" }
            }
        }
    ...
    if ($Results.ContainsKey("13")) {
        ...
        if ($dnsLines | Where-Object { $_ -notmatch "(?i)8\.8\.8\.8|8\.8\.4\.4|1\.1\.1\.1|1\.0\.0\.1|192\.168\.|10\.|172\.(1[6-9]|2[0-9]|3[01])\." }) {
            $s-=2; $i+="Serveurs DNS non standard detectes - verifier s'ils sont autorises"
        }
        if ($r -match "169\.254\.\d+\.\d+") { $s-=3; $i+="Adresse APIPA detectee (probleme DHCP)" }
        AddDomain "Configuration reseau" "cfrzo" $s $i 2
    }
    ...
    if ($Results.ContainsKey("14")) {
        ...
        if (Has "14" "(?i):445.*LISTENING|LISTENING.*:445")   { $s-=2; $i+="SMB (445) expose en ecoute" }
        if (Has "14" "(?i):5985.*LISTENING|:5986.*LISTENING") { $s-=2; $i+="WinRM (5985/5986) expose" }
        if (Has "14" "(?i):23\s|:21\s|:69\s")                 { $s-=3; $i+="Protocole non chiffre expose (Telnet/FTP/TFTP)" }
    ...
    if ($Results.ContainsKey("16")) {
        ...
        if ($r -match "(?i)WUServer\s*REG_SZ\s*(.+)") {
            ...
            if ($wsusUrl -match "^http://") { $s-=4; $i+="WSUS configure en HTTP (non chiffre) : risque WSUS-pect" }
        }
    ...
    if ($Results.ContainsKey("17")) {
        ...
        AddDomain "Mises a jour" "maj" $s $i 4
    }
    ...
    if ($Results.ContainsKey("18")) {
        $s=10; $i=@()
        $r = ResultOf "18"
        if ($r -match "(?i)Non synchronis|Not synchronized|Unsync") { $s-=3; $i+="Horloge non synchronisee (impact forensique)" }
        if ($r -match "(?i)Derive|Drift.*[5-9]\d\d\d|Drift.*[1-9]\d{4}") { $s-=2; $i+="Derive horloge importante (logs potentiellement falsifies)" }
        AddDomain "Synchronisation temps (NTP)" "ntp" $s $i 1
    }
    ...
    if ($Results.ContainsKey("19")) {
        ...
        if ($r -match "(?i)Authentification\s*:\s*Ouvrir|Authentication\s*:\s*Open") { $s-=3; $i+="Profil Wi-Fi ouvert (sans chiffrement) enregistre" }
        # WEP = obsolete
        if ($r -match "(?i)Chiffrement\s*:\s*WEP|Cipher\s*:\s*WEP") { $s-=4; $i+="Chiffrement WEP obsolete detecte" }
        ...
        if ($r -match "(?i)ProxyEnable\s*:\s*1") {
            ...
            if ($r -match "(?i)ProxyServer\s*:\s*([\d\.]+)") {
                ...
                if ($proxyIP -notmatch "^(10\.|172\.(1[6-9]|2[0-9]|3[01])\.|192\.168\.)") {
                    $s-=3; $i+="Proxy pointe vers IP publique $proxyIP (suspect)"
                }
            }
        }
    ...
    if ($Results.ContainsKey("22")) {
        ...
        if ($dupMacs.Count -gt 0) { $s-=5; $i+="Doublon(s) d'adresse MAC - possible ARP spoofing en cours" }
    ...
    if ($Results.ContainsKey("23")) {
        ...
        if ($nonComment.Count -gt 0) {
            ...
            $i += "$($nonComment.Count) entree(s) non standard dans le fichier hosts"
        }
    ...
    if ($Results.ContainsKey("24")) {
        $s=10; $i=@()
        if (Has "24" "(?i)\[Running\].*Spooler|Spooler.*Running")               { $s-=2; $i+="Spooler actif (Print-Nightmare si non patche)" }
        if (Has "24" "(?i)\[Running\].*Telnet|Telnet.*Running")                  { $s-=4; $i+="Telnet actif (protocole non chiffre)" }
    ...
    if ($Results.ContainsKey("25")) {
        ...
        if (Has "25" "(?i)State\s+OFF|Etat\s+INACTIF|Etat\s+Desactive|OFF") {
            $s-=6; $i+="Pare-feu desactive sur au moins un profil"
        }
        if (NotHas "25" "(?i)Public.*ON|Public.*ACTIF") { $s-=3; $i+="Pare-feu profil Public non confirme actif" }
    ...
    if ($Results.ContainsKey("26")) {
        ...
        if ($ruleCount -gt 300) { $s-=3; $i+="$ruleCount regles entrantes (pare-feu probablement mal configure)" }
        ...
        AddDomain "Regles FW entrantes" "infw" $s $i 2
    }

    # -- 28 Antivirus ----------------------------------------------------------
    if ($Results.ContainsKey("28")) {
        $s=10; $i=@()
        if (Has "28" "(?i)Real-time Protection Status\s*:\s*Disabled|Disabled") { $s-=5; $i+="Protection temps reel AV desactivee" }
        if (Has "28" "(?i)Definition Status\s*:\s*Out of date|Out of date")      { $s-=4; $i+="Signatures AV obsoletes" }
        if (Has "28" "(?i)Inconnu")                                               { $s-=3; $i+="Antivirus non detecte ou erreur de lecture" }
    ...
    if ($Results.ContainsKey("29")) {
        ...
        if ($suspTasks.Count -gt 5) { $s-=4; $i+="$($suspTasks.Count) taches avec chemins/exe suspects" }
        elseif ($suspTasks.Count -gt 2) { $s-=2; $i+="$($suspTasks.Count) taches avec executables systeme - verifier" }
        AddDomain "Taches planifiees" "tasks" $s $i 2
    }
    ...
    if ($Results.ContainsKey("30")) {
        $s=10; $i=@()
        $r = ResultOf "30"
        if ($r -match "(?i)AnyDesk|TeamViewer|RustDesk|UltraVNC|TightVNC|DWService") { $s-=3; $i+="Outil d'acces a distance detecte - verifier si autorise" }
        if ($r -match "(?i)Nmap|Wireshark|Metasploit|Mimikatz|Cobalt|Responder")     { $s-=5; $i+="Outil de securite/test d'intrusion detecte" }
        if ($r -match "(?i)xmrig|nicehash|cgminer|bfgminer")                         { $s-=6; $i+="Logiciel de minage de cryptomonnaie detecte" }
        if ($r -match "(?i)Tor Browser|Tor-Browser")                                  { $s-=2; $i+="Tor Browser detecte" }
        AddDomain "Logiciels installes" "logs" $s $i 3
    }
    ...
    if ($Results.ContainsKey("31")) {
        ...
        if ($errors -gt 10) { $s-=3; $i+="$errors erreurs dans le journal systeme recent" }
        elseif ($errors -gt 5) { $s-=1; $i+="$errors erreurs dans le journal systeme" }
        if ($r -match "(?i)RT2870.*invalid|Service.*invalide.*0 non valide") { $s-=1; $i+="Service(s) en erreur repetes detectes" }
        AddDomain "Journal Systeme" "syst" $s $i 1
    }
    ...
    if ($Results.ContainsKey("32")) {
        ...
        if ($errors -gt 10) { $s-=2; $i+="$errors erreurs dans le journal applicatif recent" }
        if ($r -match "(?i)Application Error.*\.exe") { $s-=2; $i+="Crash d'application detecte (verifier le processus)" }
    ...
    if ($Results.ContainsKey("33")) {
        $s=10; $i=@()
        $r = ResultOf "33"
        if ($r -match "(?i)RSOP.*vide|no data|Access.*denied") { $s-=3; $i+="GPO non appliquees ou acces refuse" }
        if (NotHas "33" "(?i)Strategie|Policy|GPO") { $s-=2; $i+="Aucune GPO de securite detectee" }
        AddDomain "Strategie de groupe (GPO)" "rsop" $s $i 2
    }
    ...
    if ($Results.ContainsKey("34")) {
        ...
        if ($suspPaths.Count -gt 0) { $s-=5; $i+="$($suspPaths.Count) entree(s) demarrage avec chemin suspect" }
        elseif ($nonEmpty.Count -gt 8) { $s-=2; $i+="$($nonEmpty.Count) programmes au demarrage - verifier les inconnus" }
        AddDomain "Programmes au demarrage" "logdem" $s $i 3
    }
    ...
    if ($Results.ContainsKey("36")) {
        $s=10; $i=@()
        $r = ResultOf "36"
        if ($r -match "(?i)non disponible|non configure") { $s-=3; $i+="BitLocker non configure ou non disponible" }
        elseif ($r -match "(?i)Protection\s*:\s*Off|ProtectionStatus.*Off") { $s-=5; $i+="BitLocker desactive sur au moins un volume" }
        elseif ($r -match "(?i)Chiffrement.*0%|EncryptionPercentage.*0") { $s-=4; $i+="Volume non chiffre detecte" }
    ...
    if ($Results.ContainsKey("37")) {
        ...
        if ($expired -gt 0)  { $s -= [math]::Min(4,$expired); $i+="$expired certificat(s) expire(s) dans les magasins" }
        if ($expiring -gt 0) { $s-=1; $i+="$expiring certificat(s) expirant dans 30j" }
        AddDomain "Certificats systeme" "certs" $s $i 2
    }
    ...
    if ($Results.ContainsKey("38")) {
        ...
        if ($newAcct -gt 0)  { $s-=4; $i+="Creation/modification de compte ou groupe detectee (4720/4732)" }
        if ($newTask -gt 0)  { $s-=3; $i+="Creation de tache planifiee detectee dans les logs (4698/4702)" }
        if ($r -match "(?i)Acces refuse|Access denied") { $s-=1; $i+="Acces refuse au journal Securite - droits insuffisants" }
        AddDomain "Journal Securite" "evtsec" $s $i 4
    }
    ...
    if ($Results.ContainsKey("39")) {
        ...
        elseif ($suspFiles.Count -gt 1) { $s-=3; $i+="$($suspFiles.Count) fichiers executables recents dans des dossiers sensibles" }
        elseif ($suspFiles.Count -eq 1) { $s-=1; $i+="1 fichier executable recent dans un dossier sensible" }
        AddDomain "Fichiers recents (72h)" "recentfiles" $s $i 3
    }
    ...
    if ($Results.ContainsKey("40")) {
        ...
        if ($rdpServers -gt 5) { $s-=2; $i+="$rdpServers serveurs RDP dans l'historique - verifier les connexions" }
    ...
    if ($Results.ContainsKey("41")) {
        $s=10; $i=@()
        $r = ResultOf "41"
        if ($r -match "(?i)non disponible|non installe|efface") { $s-=1; $i+="Historique PS absent ou efface (possible nettoyage)" }
        $suspCmds = ($r -split "`n" | Where-Object { $_ -match "(?i)\[SUSPECT\]" })
        if ($suspCmds.Count -gt 3)  { $s-=5; $i+="$($suspCmds.Count) commandes PS suspectes dans l'historique" }
    ...
    if ($Results.ContainsKey("42")) {
        ...
        AddDomain "UAC et privileges" "uac" $s $i 3
    }
    ...
    if ($Results.ContainsKey("43")) {
        $s=10; $i=@()
        $r = ResultOf "43"
        if ($r -match "(?i)AppLocker non configure|SRP non configure") { $s-=3; $i+="AppLocker/SRP non configure (execution non restreinte)" }
        if ($r -match "(?i)AppIDSvc.*Stopped|AppIDSvc.*Arrete")        { $s-=2; $i+="Service AppLocker arrete" }
        if ($r -match "(?i)aucune regle.*Exe|aucune regle.*Script")    { $s-=2; $i+="Aucune regle AppLocker sur Exe ou Script" }
    ...
    if ($Results.ContainsKey("44")) {
        $s=10; $i=@()
        $r = ResultOf "44"
        if ($r -match "(?i)\[RISQUE\] LSA RunAsPPL desactive|RunAsPPL.*0") { $s-=4; $i+="LSA RunAsPPL desactive - lsass lisible en memoire" }
        if ($r -match "(?i)\[CRITIQUE\] WDigest actif|UseLogonCredential.*1")  { $s-=5; $i+="WDigest actif - mots de passe en clair en memoire !" }
        if ($r -match "EnableVirtualizationBasedSecurity.*0") { $s-=2; $i+="Virtualization Based Security desactive" }
        AddDomain "Protection LSA / Credential Guard" "lsaprot" $s $i 4
    }
    ...
    if ($Results.ContainsKey("45")) {
        $s=10; $i=@()
        $r = ResultOf "45"
        if ($r -match "(?i)Aucune connexion") { }  # OK
        else {
            $extConns = ($r -split "`n" | Where-Object { $_ -match "\d+\.\d+\.\d+\.\d+" -and $_ -notmatch "^Date|^---" })
            if ($extConns.Count -gt 10) { $s-=2; $i+="$($extConns.Count) connexions TCP vers IPs externes" }
            $suspProcs = ($extConns | Where-Object { $_ -match "(?i)powershell|cmd|wscript|cscript|mshta|rundll32" })
            if ($suspProcs.Count -gt 0) { $s-=5; $i+="Connexion(s) externe(s) via processus suspect (PS/cmd/mshta)" }
        }
    ...
    if ($Results.ContainsKey("46")) {
        $s=10; $i=@()
        $r = ResultOf "46"
        if ($r -match "(?i)Script Block Logging.*INACTIF") { $s-=2; $i+="Script Block Logging PS desactive (detection reduite)" }
        if ($r -match "(?i)Module Logging.*INACTIF")       { $s-=1; $i+="Module Logging PS desactive" }
        if ($r -match "(?i)Transcription.*INACTIF")        { $s-=1; $i+="Transcription PS desactivee" }
    ...
    if ($Results.ContainsKey("04")) {
        ...
        if ($r -match "(?i)svchost.*ppid=(?!4|8|688|772|804|824)") { $s-=3; $i+="svchost.exe avec parent inhabituel detecte" }
    ...
    if ($Results.ContainsKey("15")) {
        ...
        if ($suspRoutes.Count -gt 3) { $s-=2; $i+="$($suspRoutes.Count) routes statiques non standard - verifier si legitimes" }
    ...
    if ($Results.ContainsKey("20")) {
        ...
        if ($suspDomains.Count -gt 0) { $s-=3; $i+="$($suspDomains.Count) domaine(s) au format DGA potentiel dans le cache DNS" }
    ...
    if ($Results.ContainsKey("27")) {
        ...
        $r = ResultOf "27"
        $ruleCount = ([regex]::Matches($r,"(?m)^\[Allow\]|\[Block\]")).Count
        if ($ruleCount -gt 300) { $s-=2; $i+="$ruleCount regles sortantes (pare-feu peu restrictif)" }
        # Pas de regles = tout autorise en sortie
        if ($ruleCount -eq 0) { $s-=1; $i+="Aucune regle sortante : tout trafic sortant autorise" }
        AddDomain "Regles FW sortantes" "outfw" $s $i 1
    }
    ...
    # Plafonds punitifs selon gravite des domaines critiques en echec
    # Seuils : <5 = Critique, <7 = Attention, >=7 = Bon
...
# ---------------------------------------------------------------------------
#  MATRICE MITRE ATT&CK WINDOWS - Generation HTML
# ---------------------------------------------------------------------------
...
function Generate-MitreMatrix {
    ...
    $F.WDigest        = (Has "44" "(?i)WDigest.*1|UseLogonCredential.*1|\[CRITIQUE\] WDigest")
    ...
    $F.ExtConns       = ($Results.ContainsKey("45") -and (ResultOf "45") -notmatch "(?i)Aucune connexion")
    $F.NoFWOut        = (Has "27" "(?i)Aucune regle sortante")
    ...
    for ($row = 0; $row -lt $maxRows; $row++) {
        ...
        foreach ($tac in $tactics.Keys) {
            ...
            if ($row -lt $tacTechs.Count) {
                ...
                $tip  = switch ($tlvl) { 3 {"CRITIQUE - facilite par les vuln. detectees"} 2 {"ELEVE - conditions favorables"} 1 {"POSSIBLE - conditions partielles"} default {"Non facilite"} }
                ...
                if ($internalAnchor) {
                    $bodyRows += "<td class='tech-cell $cls' title='$tid - $tip'>" +
                        ...
                        "<a href='#$internalAnchor' class='tech-ext-link' onclick='event.preventDefault();navToAnchor(`"$internalAnchor`")' title='Voir dans le rapport'>&#8595;</a>" +
                        "</td>`n"
                } else {
    ...
    $activeFlags = ""
    $flagLabels = @{
        NoLockout     = "Pas de verrouillage compte (brute-force)"
        WeakPwPolicy  = "Politique mdp faible"
        AdminActive   = "Compte Admin integre actif"
        NoPassword    = "Compte sans mot de passe"
        WDigest       = "WDigest actif (MDP en memoire)"
        NoLSAPPL      = "LSA RunAsPPL desactive"
        PSUnrestricted= "PowerShell non restreint"
        NoPSLogging   = "Journalisation PS inactive"
        NoAppLocker   = "AppLocker non configure"
        SuspStartup   = "Entrees demarrage suspectes"
        SuspTasks     = "Taches planifiees suspectes"
        HostsModified = "Fichier HOSTS modifie"
        UACDisabled   = "UAC desactive"
        UACSilent     = "Elevation UAC silencieuse"
        SeDebug       = "SeDebugPrivilege actif"
        RDPOpen       = "RDP expose (port 3389)"
        SMBOpen       = "SMB expose (port 445)"
        WinRMOpen     = "WinRM expose (5985/5986)"
        AdminShares   = "Partages admin actifs"
        FWDisabled    = "Pare-feu desactive"
        AVDisabled    = "AV desactive/obsolete"
        NoUpdates     = "MAJ critiques manquantes"
        NoBitLocker   = "Pas de chiffrement disque"
        WifiOpen      = "Wi-Fi ouvert/WEP"
        ...
        ExtConns      = "Connexions externes actives"
        NoFWOut       = "Aucune regle FW sortante"
        WSUSHttp      = "WSUS en HTTP (non chiffre)"
        NoCredGuard   = "Credential Guard desactive"
        SuspFiles     = "Fichiers executables suspects"
        SuspPSHist    = "Historique PS suspect"
        ManyPorts     = "Nombreux ports en ecoute"
        SNMPActive    = "SNMP actif"
        BootDebug     = "Mode debug/test boot actif"
        RDPHistory    = "Historique RDP (connexions)"
        AdminGroup    = "Groupe Admins surpeuple"
    ...
    foreach ($fk in ($F.Keys | Sort-Object)) {
        if ($F[$fk] -eq $true) {
            ...
            if ($anc) {
                $activeFlags += "<a href='#$anc' class='flag-item flag-link' onclick='event.preventDefault();navToAnchor(`"$anc`")' title='Aller a la section'>$lbl <span class='flag-arrow'>&#8594;</span></a>`n"
            } else {
                $activeFlags += "<span class='flag-item'>$lbl</span>`n"
            }
        }
    }
    if (-not $activeFlags) { $activeFlags = "<span class='flag-none'>Aucune vulnerabilite significative detectee</span>" }
...
<section id='mitre-matrix' class='audit-section mitre-section'>
  <div class='section-header'>
    ...
    <span class='cat-pill'>$affectedCount / $totalTech techniques impactees</span>
  </div>
  ...
  <div class='mitre-body'>
    ...
    <div class='mitre-stats'>
      <div class='ms-card crit'><div class='ms-num'>$critCount</div><div class='ms-lbl'>Critiques</div></div>
      <div class='ms-card warn'><div class='ms-num'>$warnCount</div><div class='ms-lbl'>Elevees</div></div>
      <div class='ms-card poss'><div class='ms-num'>$possCount</div><div class='ms-lbl'>Possibles</div></div>
      <div class='ms-card none'><div class='ms-num'>$($totalTech - $affectedCount)</div><div class='ms-lbl'>Non concernees</div></div>
    </div>

    <!-- Legende couleurs -->
    <div class='mitre-legend'>
      <span class='leg-item t-crit'>Critique - vuln. detectees</span>
      <span class='leg-item t-warn'>Eleve - conditions favorables</span>
      <span class='leg-item t-poss'>Possible - conditions partielles</span>
      <span class='leg-item t-none'>Non facilite</span>
    </div>

    <!-- Vulnerabilites actives -->
    <div class='mitre-flags'>
      <div class='flags-title'>Vulnerabilites detectees contribuant a la matrice :</div>
    ...
    <div class='mitre-footer'>
      Basee sur MITRE ATT&amp;CK v14 Enterprise / Windows. Cliquer sur une technique pour ouvrir la fiche ATT&amp;CK.
...
function Generate-RemediationPlan {
    ...
    # Chaque action : @{ Prio(1=critique,2=haute,3=moyenne); Cat; Titre; Detail; Commande; Ref }
    ...
    if (Has "08" "(?i)\[!MOT DE PASSE VIDE\]") {
        Add-Action 1 "Comptes" "Definir un mot de passe sur tous les comptes actifs" `
            "Des comptes sans mot de passe permettent un acces immediat sans credential." `
    ...
    if (Has "08" "(?i)(Administrator|Administrateur).*Actif\s*:\s*True") {
        Add-Action 1 "Comptes" "Renommer et desactiver le compte Administrateur integre" `
...
Disable-LocalUser -Name "Administrateur"' `
    ...
    if (Has "08" "(?i)(Guest|Invite).*Actif\s*:\s*True") {
        Add-Action 1 "Comptes" "Desactiver le compte Invite/Guest" `
...
Disable-LocalUser -Name "Guest"' `
    ...
    if (Has "07" "LockoutBadCount\s*=\s*0") {
        Add-Action 1 "Comptes" "Activer le verrouillage de compte apres echecs" `
            "Sans verrouillage, les attaques par brute-force peuvent tester des millions de mots de passe." `
    ...
    if (Has "07" "PasswordComplexity\s*=\s*0") {
        Add-Action 1 "Comptes" "Activer la complexite des mots de passe" `
            "Sans complexite, les mots de passe simples (123456, prenom) sont courants." `
...
secedit /configure /db secedit.sdb /cfg C:\temp\secpol.cfg /overwrite' `
    ...
    if (Has "07" "MinimumPasswordLength\s*=\s*[0-9]\b") {
        ...
        if ($pwlen -lt 12) {
            Add-Action 2 "Comptes" "Augmenter la longueur minimale des mots de passe a 12" `
                "Longueur actuelle : $pwlen. Un mot de passe de 12+ chars resiste aux attaques par dictionnaire." `
    ...
    if (Has "25" "(?i)State\s+OFF|Etat\s+INACTIF|OFF") {
        Add-Action 1 "Pare-feu" "Reactiver le pare-feu Windows sur tous les profils" `
            "Un pare-feu desactive laisse tous les ports exposes sans filtrage." `
    ...
    if (Has "14" "(?i):3389.*LISTENING|LISTENING.*:3389") {
        Add-Action 1 "Pare-feu" "Restreindre l'acces RDP aux seules IPs autorisees" `
            "RDP expose a tous est la principale porte d'entree des attaques par brute-force et des ransomwares." `
...
New-NetFirewallRule -DisplayName "RDP - Bloquer tout" -Direction Inbound -Protocol TCP -LocalPort 3389 -Action Block' `
    ...
    if (Has "14" "(?i):445.*LISTENING|LISTENING.*:445") {
        Add-Action 1 "Pare-feu" "Bloquer le port SMB (445) en entree depuis Internet" `
            "SMB expose sur Internet permet l'exploitation de vuln. critiques (Eternal-Blue, etc.)." `
    ...
    if (Has "14" "(?i):5985.*LISTENING|:5986.*LISTENING") {
        Add-Action 2 "Pare-feu" "Restreindre WinRM aux seules IPs d'administration" `
            "WinRM permet l'execution de commandes PowerShell a distance." `
    ...
    if ($Results.ContainsKey("17")) {
        ...
        if ($daysSince -gt 30) {
            Add-Action 1 "MAJ" "Installer les mises a jour Windows en attente ($daysSince jours de retard)" `
                "Chaque jour sans MAJ expose le systeme aux CVE publiques et aux outils d'exploitation automatiques." `
...
Get-WindowsUpdate -Install -AcceptAll -AutoReboot' `
    ...
    if (Has "44" "(?i)RunAsPPL.*0|\[RISQUE\] LSA") {
        Add-Action 1 "Credentials" "Activer LSA Protected Process Light (RunAsPPL)" `
            "Sans RunAsPPL, lsass.exe est lisible par tout processus admin, permettant l'extraction de credentials." `
...
# Redemarrage requis' `
    ...
    if (Has "44" "(?i)UseLogonCredential.*1|\[CRITIQUE\] WDigest") {
        Add-Action 1 "Credentials" "Desactiver WDigest pour eliminer les mots de passe en clair en memoire" `
            "WDigest force Windows a conserver les credentials en clair dans lsass (heritage Windows XP)." `
    ...
    if (Has "44" "EnableVirtualizationBasedSecurity.*0") {
        Add-Action 2 "Credentials" "Activer Credential Guard (VBS)" `
            "Credential Guard isole les credentials dans un environnement virtualise inaccessible depuis l'OS." `
...
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Lsa" /v LsaCfgFlags /t REG_DWORD /d 1 /f' `
    ...
    if (Has "42" "(?i)EnableLUA.*0|\[RISQUE\] UAC desactive") {
        Add-Action 1 "Privileges" "Reactiver l'UAC (User Account Control)" `
            "UAC desactive permet a tout processus d'obtenir les privileges SYSTEM sans demande de confirmation." `
...
# Redemarrage requis' `
    ...
    if (Has "42" "(?i)ConsentPromptBehaviorAdmin.*0") {
        Add-Action 1 "Privileges" "Configurer UAC pour demander confirmation (pas d'elevation silencieuse)" `
            "L'elevation silencieuse permet aux programmes malveillants de s'elever sans alerte utilisateur." `
    ...
    if (Has "36" "(?i)desactive|non configure|Off") {
        Add-Action 1 "Chiffrement" "Activer BitLocker sur le volume systeme" `
            "Un disque non chiffre peut etre lu integralement en cas de vol (boot sur USB, extraction)." `
    ...
    if (Has "28" "(?i)Disabled|Out of date") {
        Add-Action 1 "AV/EDR" "Reactiver et mettre a jour l'antivirus" `
            "Un AV desactive ou obsolete n'offre aucune protection contre les menaces recentes." `
...
Update-MpSignature' `
    ...
    if (Has "06" "(?i)Unrestricted|Bypass") {
        Add-Action 1 "PowerShell" "Restreindre la politique d'execution PowerShell" `
            "Unrestricted/Bypass permet l'execution de tout script PS, vecteur majeur d'attaques sans fichier." `
...
Set-ExecutionPolicy AllSigned -Scope CurrentUser -Force' `
    ...
    if (Has "46" "(?i)Script Block Logging.*INACTIF") {
        Add-Action 2 "PowerShell" "Activer le Script Block Logging PowerShell" `
            "Le Script Block Logging enregistre tout le code PS execute, y compris les commandes decodees (base64)." `
    ...
    if (Has "46" "(?i)Module Logging.*INACTIF") {
        Add-Action 3 "PowerShell" "Activer le Module Logging PowerShell" `
            "Le Module Logging enregistre les appels de fonctions de tous les modules PS charges." `
    ...
    if (Has "43" "(?i)AppLocker non configure|aucune regle.*Exe") {
        Add-Action 2 "Controle execution" "Configurer AppLocker pour restreindre l'execution" `
...
Set-Service AppIDSvc -StartupType Automatic' `
    ...
    if (Has "24" "(?i)\[Running\].*Telnet|Telnet.*Running") {
        Add-Action 1 "Services" "Desactiver le service Telnet" `
            "Telnet transmet les credentials en clair sur le reseau. Remplacer par SSH." `
...
Set-Service -Name Telnet -StartupType Disabled' `
    ...
    if (Has "24" "(?i)\[Running\].*RemoteRegistry|RemoteRegistry.*Running") {
        Add-Action 2 "Services" "Desactiver le service RemoteRegistry" `
            "RemoteRegistry permet la lecture/ecriture du registre depuis le reseau, vecteur de reconnaissance." `
...
Set-Service -Name RemoteRegistry -StartupType Disabled' `
    ...
    if (Has "24" "(?i)\[Running\].*SNMP|SNMP.*Running") {
        Add-Action 2 "Services" "Desactiver SNMP ou securiser la communaute" `
            "SNMP v1/v2 avec la communaute 'public' expose des informations systeme a tout le reseau." `
...
Set-Service -Name SNMP -StartupType Disabled' `
    ...
    # ?? Partages ????????????????????????????????????????????????????????????
    if (Has "10" "(?i)ADMIN\`\$|C\`\$") {
        Add-Action 2 "Reseau" "Desactiver les partages administratifs caches" `
            "Les partages admin (C`$, ADMIN`$) sont exploites par les ransom-wares pour se propager." `
...
net share ADMIN$ /delete' `
    ...
    if (Has "23" "(?i)[^#\n]+\d+\.\d+\.\d+\.\d+") {
        ...
        if ($hostsLines.Count -gt 0) {
            Add-Action 1 "Reseau" "Nettoyer le fichier HOSTS des entrees non autorisees" `
...
notepad "$env:SystemRoot\System32\drivers\etc\hosts"' `
    ...
    # ?? Journalisation ??????????????????????????????????????????????????????
    if ($Results.ContainsKey("46")) {
        Add-Action 2 "Journalisation" "Augmenter la taille des journaux d'evenements" `
            "Les journaux trop petits sont ecrases rapidement, detruisant les preuves forensiques." `
...
wevtutil sl Application /ms:524288000' `
    ...
    if (Has "46" "(?i)Transcription.*INACTIF") {
        Add-Action 3 "Journalisation" "Activer la transcription PowerShell" `
            "La transcription enregistre toutes les sessions PS dans des fichiers logs horodates." `
...
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\PowerShell\Transcription" /v OutputDirectory /t REG_SZ /d "C:\PSLogs" /f' `
    ...
    if (Has "35" "(?i)Teredo|6to4|ISATAP") {
        Add-Action 3 "Reseau" "Desactiver les tunnels IPv6 (Teredo, 6to4, ISATAP)" `
            "Ces tunnels encapsulent IPv6 dans IPv4 et contournent les pare-feux qui ne filtrent pas IPv6." `
...
netsh interface isatap set state disabled' `
    ...
    if (Has "19" "(?i)WEP|Open.*Authentification|Authentication.*Open") {
        Add-Action 1 "Wi-Fi" "Supprimer les profils Wi-Fi avec chiffrement faible (WEP/ouvert)" `
            "WEP est casse en quelques minutes. Les reseaux ouverts exposent tout le trafic en clair." `
            '# Lister les profils :
...
netsh wlan delete profile name="<NomDuProfil>"' `
    ...
    # ?? ARP / Reseau ????????????????????????????????????????????????????????
    if (Has "22" "(?i)Doublon") {
        Add-Action 1 "Reseau" "Investiguer une anomalie ARP (possible ARP spoofing)" `
            "Un doublon d'adresse MAC dans la table ARP peut indiquer une attaque man-in-the-middle en cours." `
...
# Identifier la machine avec l adresse MAC dupliquee' `
    ...
    if (Has "21" "(?i)ProxyEnable.*1") {
        Add-Action 2 "Reseau" "Verifier et valider la configuration proxy" `
            "Un proxy configure a l'insu de l'utilisateur permet l'intercepton de tout le trafic HTTP/HTTPS." `
...
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings" -Name ProxyEnable -Value 0' `
    ...
    if (Has "34" "(?i)temp|appdata|public") {
        Add-Action 1 "Persistance" "Investiguer et nettoyer les entrees de demarrage suspectes" `
            "Des programmes se lancant depuis TEMP ou APPDATA sont caracteristiques de logiciels suspects." `
            '# Lister toutes les entrees Run :
Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run"
Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
# Supprimer une entree suspecte :
Remove-ItemProperty -Path "HKCU:\...\Run" -Name "<NomEntree>"' `
            "MITRE T1547.001" `
            "logdem"
    }

    if (Has "29" "(?i)\\\\Temp\\\\|\\\\AppData\\\\") {
        Add-Action 1 "Persistance" "Auditer et supprimer les taches planifiees suspectes" `
            "Des taches executant des scripts depuis TEMP ou APPDATA sont un signe de persistence suspecte." `
...
Unregister-ScheduledTask -TaskName "<Nom>" -Confirm:$false' `
    ...
    # ?? Journal securite ????????????????????????????????????????????????????
    if (Has "38" "(?i)4625") {
        Add-Action 1 "Forensique" "Investiguer les echecs de connexion (EventID 4625)" `
            "Des echecs de connexion en masse indiquent un brute-force actif. Identifier l'IP source." `
...
Get-EventLog -LogName Security -InstanceId 4625 -Newest 50 | Select-Object TimeGenerated,Message | Format-List' `
    ...
    if (Has "38" "(?i)4720|4732") {
        Add-Action 1 "Forensique" "Investiguer la creation/modification de comptes (4720/4732)" `
            "Un compte cree ou ajoute a un groupe sans autorisation indique une persistence." `
    ...
    # ?? WSUS HTTP ???????????????????????????????????????????????????????????
    if (Has "16" "(?i)http://") {
        Add-Action 1 "MAJ" "Securiser la communication WSUS en HTTPS" `
            "Un serveur WSUS en HTTP permet des attaques WSUS-pect : distribution de fausses MAJ." `
...
# Sur les clients GPO : WUServer = https://<serveur>:8531' `
    ...
    if (Has "41" "(?i)\[SUSPECT\]") {
        Add-Action 1 "Forensique" "Analyser les commandes PowerShell suspectes dans l'historique" `
            "Des commandes avec -enc, DownloadString ou IEX sont caracteristiques d'une attaque sans fichier." `
...
Get-WinEvent -LogName "Microsoft-Windows-PowerShell/Operational" | Where-Object Id -eq 4104' `
    ...
    function Build-ActionCards {
        ...
        foreach ($a in $list) {
            ...
            $anchorHtml = if ($a.Anchor) { "<a href='#$($a.Anchor)' class='rem-section-link' onclick='event.preventDefault();navToAnchor(`"$($a.Anchor)`")'>&rarr; Voir la section</a>" } else { "" }
            $html += @"
...
"@
    ...
    if (-not $cards1 -and -not $cards2 -and -not $cards3) {
        $cards1 = "<div class='rem-empty'>Aucune vulnerabilite majeure detectee automatiquement. Consulter les conseils de chaque section.</div>"
    }
  ...
  <div class='rem-body-wrap'>
    ...
    <div class='rem-summary'>
      <div class='rem-sum-card p1'>
        ...
        <span class='rem-sum-lbl'>Critiques</span>
      </div>
      <div class='rem-sum-card p2'>
        <span class='rem-sum-num'>$($p2.Count)</span>
        <span class='rem-sum-lbl'>Haute priorite</span>
      </div>
      <div class='rem-sum-card p3'>
        <span class='rem-sum-num'>$($p3.Count)</span>
        <span class='rem-sum-lbl'>Moyenne priorite</span>
      </div>
      <div class='rem-disclaimer'>
        Ces remediations sont generees automatiquement a partir des elements detectes.
        Tester en environnement de validation avant tout deploiement en production.
        Certaines actions necessitent un redemarrage.
      </div>
    </div>

    $(if ($cards1) { "<div class='rem-group-title p1-title'>Actions critiques - a traiter en priorite</div>$cards1" })
    $(if ($cards2) { "<div class='rem-group-title p2-title'>Actions haute priorite</div>$cards2" })
    $(if ($cards3) { "<div class='rem-group-title p3-title'>Actions de durcissement</div>$cards3" })
...
function Build-HtmlReport {
    ...
    $scoreLabel = if ($globalScore -ge 7) { "Bon" } elseif ($globalScore -ge 5) { "Attention" } else { "Critique" }
    ...
    foreach ($d in $sortedDomains) {
        ...
        $dlabel = if ($ds -ge 8) { "OK" } elseif ($ds -ge 5) { "Attention" } else { "Critique" }
        ...
        if ($d.Issues.Count -gt 0) {
            ...
            $issuesHtml = "<p class='domain-ok'>Aucun probleme detecte</p>"
        }
        $domainCards += @"
...
<section id='dashboard' class='audit-section dashboard-section'>
  <div class='section-header'>
    <h2>Tableau de bord de securite</h2>
  ...
  <div class='dashboard-body'>
    ...
    <div class='global-score-wrap'>
      ...
      <div class='global-meta'>
        <div class='gm-title'>Score de securite global</div>
        <div class='gm-machine'>$computername</div>
        <div class='gm-date'>Audit du $date a $hour</div>
        <div class='gm-modules'>$modCount modules analyses</div>
        <div class='gm-legend'>
          <span class='leg crit'>0-4 Critique</span>
          <span class='leg warn'>5-6 Attention</span>
          <span class='leg ok'>7-10 Bon</span>
        </div>
      </div>
    </div>

    <!-- Grille des domaines -->
    <div class='domains-title'>Analyse par domaine <span class='domains-hint'>-- Cliquer pour acceder a la section</span></div>
...
"@
    ...
    foreach ($key in ($Results.Keys | Sort-Object)) {
        ...
        if ($mConseil) {
            ...
            if     ($mConseil -match "^\[CRITIQUE\]")  { $cClass="critique";  $cLabel="CRITIQUE";  $cText=$mConseil -replace "^\[CRITIQUE\]\s*","" }
...
<div id="sidebar">
  <div id="sidebar-top">
    ...
    <div style="display:flex;gap:5px">
      <button id="expand-all">Tout ouvrir</button>
      <button id="collapse-all">Tout fermer</button>
    </div>
  </div>
  <nav>
    <ul id="nav-list">
    ...
    <li><a class='nav-link rem-link' href='#remediation' data-anchor='remediation'>Plan de remediation</a></li>
$menuItems
    </ul>
  </nav>
  ...
  <div id="sidebar-refs">
    <button id="refs-toggle" class="refs-toggle-btn">
      ...
      <span class="refs-title-text">References de securite</span>
      <span class="refs-count">12</span>
    </button>
    <div id="refs-list" style="display:none">

    <a class="ref-link anssi" href="https://cyber.gouv.fr/publications/guide-dhygiene-informatique" target="_blank" rel="noopener">
      <span class="ref-icon">AN</span>
      <span class="ref-body">
        <span class="ref-name">Guide Hygiene informatique</span>
        <span class="ref-org">ANSSI -- 42 regles essentielles</span>
      </span>
    </a>

    <a class="ref-link anssi" href="https://cyber.gouv.fr/publications/recommandations-de-configuration-materielle-de-postes-clients-et-serveurs-x86" target="_blank" rel="noopener">
      <span class="ref-icon">AN</span>
      <span class="ref-body">
        <span class="ref-name">Config. postes clients x86</span>
        <span class="ref-org">ANSSI -- Durcissement materiel</span>
      </span>
    </a>

    <a class="ref-link anssi" href="https://cyber.gouv.fr/publications/recommandations-relatives-lauthentification-multifacteur-et-aux-mots-de-passe" target="_blank" rel="noopener">
      <span class="ref-icon">AN</span>
      <span class="ref-body">
        <span class="ref-name">MFA et mots de passe</span>
        <span class="ref-org">ANSSI -- Recommandations auth</span>
      </span>
    </a>

    <a class="ref-link anssi" href="https://cyber.gouv.fr/publications/mettre-en-oeuvre-une-politique-de-restrictions-logicielles-sous-windows" target="_blank" rel="noopener">
      <span class="ref-icon">AN</span>
      <span class="ref-body">
        <span class="ref-name">SRP / AppLocker Windows</span>
        <span class="ref-org">ANSSI -- Restrictions logicielles</span>
      </span>
    </a>

    <a class="ref-link anssi" href="https://cyber.gouv.fr/publications" target="_blank" rel="noopener">
      <span class="ref-icon">AN</span>
      <span class="ref-body">
        <span class="ref-name">Toutes les publications</span>
        <span class="ref-org">ANSSI -- Catalogue complet</span>
      </span>
    </a>
    ...
    <a class="ref-link cis" href="https://www.cisecurity.org/cis-benchmarks" target="_blank" rel="noopener">
      ...
      <span class="ref-body">
        ...
        <span class="ref-org">CIS -- Durcissement Windows 10/11</span>
      </span>
    </a>
    ...
    <a class="ref-link cis" href="https://www.cisecurity.org/controls" target="_blank" rel="noopener">
      ...
      <span class="ref-body">
        ...
        <span class="ref-org">CIS -- 18 controles de securite</span>
      </span>
    </a>
    ...
    <a class="ref-link nist" href="https://csrc.nist.gov/pubs/sp/800/171/r3/final" target="_blank" rel="noopener">
      ...
      <span class="ref-body">
        ...
        <span class="ref-org">NIST -- Protection donnees sensibles</span>
      </span>
    </a>
    ...
    <a class="ref-link mitre" href="https://attack.mitre.org/matrices/enterprise/windows/" target="_blank" rel="noopener">
      ...
      <span class="ref-body">
        ...
        <span class="ref-org">MITRE -- Tactiques et techniques</span>
      </span>
    </a>
    ...
    <a class="ref-link mitre" href="https://attack.mitre.org/mitigations/enterprise/" target="_blank" rel="noopener">
      ...
      <span class="ref-body">
        ...
        <span class="ref-org">MITRE -- Contre-mesures par technique</span>
      </span>
    </a>

    <a class="ref-link ssi" href="https://learn.microsoft.com/fr-fr/windows/security/operating-system-security/device-management/windows-security-configuration-framework/windows-security-baselines" target="_blank" rel="noopener">
      <span class="ref-icon">MS</span>
      <span class="ref-body">
        <span class="ref-name">Microsoft Security Baselines</span>
        <span class="ref-org">Microsoft -- MSCT baselines</span>
      </span>
    </a>

    <a class="ref-link ssi" href="https://www.cert.ssi.gouv.fr/" target="_blank" rel="noopener">
      <span class="ref-icon">CE</span>
      <span class="ref-body">
        <span class="ref-name">CERT-FR Alertes et avis</span>
        <span class="ref-org">ANSSI CERT-FR -- Menaces actives</span>
      </span>
    </a>
...
<!-- BACK TO TOP -->
<button id="back-to-top" title="Haut de page">&#8679;</button>
...
function Show-MainMenu {
    ...
    Write-Host "    [2]  Audit selectif  -- choisir les modules" -ForegroundColor Green
    Write-Host ""
    Write-Host "  INVESTIGATION" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "    [3]  Executer un module -- resultat dans le shell" -ForegroundColor Cyan
    Write-Host "    [4]  Ouvrir le dernier rapport HTML" -ForegroundColor Cyan
    Write-Host "    [5]  Resume complet de la machine" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  ----------------------------------------------------------------------" -ForegroundColor DarkGray
    Write-Host ""
...
function Run-Menu2 {
    ...
    while ($true) {
        ...
        Write-Host "  |  MENU 2 -- SELECTION DES MODULES POUR RAPPORT PERSONNALISE       |" -ForegroundColor Cyan
        Write-Host "  +==================================================================+" -ForegroundColor Cyan
        Write-Host ""
        Write-Host "  Commandes : [numero] cocher/decocher  |  A=tout  |  N=aucun  |  V=valider & generer  |  Q=retour" -ForegroundColor DarkGray
        Write-Host ("  " + ("-" * 90))
        Write-Host ""
        ...
        Write-Host ""
        Write-Host ("  Modules selectionnes : {0}/{1}   (V=valider et generer le rapport  |  Q=retour sans generer)" -f $Global:Menu2Selection.Count, $Global:AuditModules.Count) -ForegroundColor Yellow
        Write-Host ""
        ...
        elseif ($cmd -eq "V" -or $cmd -eq "v") {
            if ($Global:Menu2Selection.Count -eq 0) {
                Write-Host "  Aucun module selectionne." -ForegroundColor Red
        ...
        elseif ($Global:AuditModules.Contains($cmd)) {
            if ($Global:Menu2Selection -contains $cmd) {
                ...
                Write-Host ("  [-] {0} retire" -f $cmd) -ForegroundColor DarkGray
            } else {
                $Global:Menu2Selection += $cmd
                Write-Host ("  [+] {0} ajoute" -f $cmd) -ForegroundColor Green
            }
...
function Run-Menu3 {
    while ($true) {
        ...
        Write-Host ""
        Write-Host "  +==================================================================+" -ForegroundColor Cyan
        Write-Host "  |  MENU 3 -- EXECUTION INDIVIDUELLE (resultat dans le shell)       |" -ForegroundColor Cyan
        Write-Host "  +==================================================================+" -ForegroundColor Cyan
        Write-Host ""
        $currentCategory = ""
        foreach ($key in $Global:AuditModules.Keys) {
        ...
        $cmd = (Read-Host "  Numero du module (Q=retour menu principal)").Trim()
        ...
        elseif ($Global:AuditModules.Contains($cmd)) {
            ...
            Write-Host ("  Module termine en {0:F1}s" -f $elapsed) -ForegroundColor Yellow
            Write-Host ("  " + ("=" * 68)) -ForegroundColor DarkCyan
            Write-Host ""
            Read-Host "  Appuyez sur Entree pour revenir a la liste"
        }
        else {
            Write-Host ("  '{0}' non reconnu." -f $cmd) -ForegroundColor Red
            Start-Sleep -Milliseconds 700
...
function Run-Modules {
    param(
        ...
        [string]$ReportType = "rapport_complet"
    )
    ...
    Write-Host ""
    Write-Host "  Demarrage de l'audit -- $total module(s) selectionne(s)" -ForegroundColor Yellow
    Write-Host ""
    ...
    foreach ($key in $sortedKeys) {
        ...
        try {
            $output      = & $m.Script
            $results[$key] = if ($output) { $output | Out-String } else { "(Aucun resultat)" }
        } catch {
            Write-Host "  [ERREUR module $key] $($_.Exception.Message)" -ForegroundColor Red
            $results[$key] = "[ERREUR] $($_.Exception.Message)"
        }
        $elapsed = ((Get-Date) - $startTime).TotalSeconds
        Write-Host ("  OK {0} -- termine en {1:F1}s" -f $m.Name, $elapsed) -ForegroundColor DarkGreen
    }
    ...
    if ($GenerateHtml -and $results.Count -gt 0) {
        ...
        Write-Host "  Rapport enregistre : $reportDir" -ForegroundColor Green
    }
...
while ($true) {
    ...
    switch ($choice) {
        "1" {
            Write-Host ""
            Write-Host "  Lancement de l'audit complet ($($Global:AuditModules.Count) modules)..." -ForegroundColor Green
            Write-Host ""
            $allKeys = [string[]]($Global:AuditModules.Keys | Sort-Object)
            Run-Modules -Keys $allKeys -GenerateHtml $true -ReportType "rapport_complet"
            Write-Host ""
        ...
        "2" {
            ...
            if ($null -ne $selectedSet -and $selectedSet.Count -gt 0) {
                Write-Host ""
                Write-Host "  $($selectedSet.Count) module(s) selectionne(s). Demarrage..." -ForegroundColor Yellow
                Run-Modules -Keys ([string[]]$selectedSet) -GenerateHtml $true -ReportType "rapport_partiel"
                Write-Host ""
                Read-Host "  Appuyez sur Entree pour revenir au menu"
        ...
        "4" {
            ...
            if ($Global:LastReportPath -and (Test-Path $Global:LastReportPath)) {
                ...
                Write-Host ""
                Write-Host "  Aucun rapport trouve. Lancez d'abord un audit (option 1 ou 2)." -ForegroundColor Red
        ...
        "5" {
            Clear-Host
            Show-Banner
            Write-Host "  ---- RESUME COMPLET DE LA MACHINE ----------------------------------" -ForegroundColor Cyan
            Write-Host ""

            # --- SYSTEME ---
            Write-Host ""
            Write-Host "  ---- SYSTEME --------------------------------------------------------" -ForegroundColor DarkYellow
            Write-Host ""
            try {
                $os      = Get-CimInstance Win32_OperatingSystem
                $cs      = Get-CimInstance Win32_ComputerSystem
            ...
            # --- CLE WINDOWS ---
            Write-Host ""
            Write-Host "  ---- CLE DE LICENCE WINDOWS -----------------------------------------" -ForegroundColor DarkYellow
            Write-Host ""
            try {
                $key = (Get-WmiObject -query 'select * from SoftwareLicensingService').OA3xOriginalProductKey
                if ($key) {
                    Write-Host ("    Cle OEM          : {0}" -f $key) -ForegroundColor Green
                } else {
                    ...
                    if ($digitalId) {
                        ...
                        Write-Host ("    Cle produit      : {0}" -f $productKey) -ForegroundColor Green
                    } else {
                        Write-Host "    Cle non trouvee (licence numerique ou non disponible)" -ForegroundColor DarkGray
                    }
                }
            } catch {
                Write-Host "    [ERREUR] $($_.Exception.Message)" -ForegroundColor Red
            ...
            # --- WIFI ---
            Write-Host ""
            Write-Host "  ---- RESEAUX WIFI SAUVEGARDES ---------------------------------------" -ForegroundColor DarkYellow
            Write-Host ""
            try {
                ...
                if ($profiles) {
                    foreach ($p in $profiles) {
                        ...
                        $pwd    = ($detail | Select-String "Contenu de la cl|Key Content") |
                            ForEach-Object { ($_ -split ":")[1].Trim() }
                        if (-not $pwd) { $pwd = "(mot de passe non disponible)" }
                        Write-Host ("    SSID  : {0}" -f $p)   -ForegroundColor Cyan
                        Write-Host ("    Mdp   : {0}" -f $pwd) -ForegroundColor Yellow
                        Write-Host ""
                    }
                } else {
                    Write-Host "    Aucun profil WiFi sauvegarde." -ForegroundColor DarkGray
                }
            } catch {
                Write-Host "    [ERREUR] $($_.Exception.Message)" -ForegroundColor Red
            }
            ...
            if ($Global:LastReportPath -and (Test-Path $Global:LastReportPath)) {
                ...
                Write-Host ("    Genere le        : {0}" -f $rapportDate)            -ForegroundColor DarkCyan
            } else {
                Write-Host "    Aucun rapport HTML genere pour cette machine." -ForegroundColor DarkGray
            }
