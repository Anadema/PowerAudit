...
if (-not $isAdmin) {
    ...
    Write-Host "  Veuillez relancer PowerShell avec les droits administrateur et réexécuter le script." -ForegroundColor Yellow
    Write-Host ""
...
$Global:AuditModules = [ordered]@{
    ...
    "01" = @{
        ...
        ShortName  = "BCD"
        Category   = "Système"
        Conseil    = "[CRITIQUE] Vérifier les entrées de démarrage inconnues ou non signées (dual-boot suspect, boot-kits). Un boot-kit (ex. rootkit MBR) peut persister avant le chargement de Windows et échapper à l'AV. Vérifier que SecureBoot est actif (bcdedit /enum firmware). Toute entrée avec un chemin inhabituel vers winload.exe est suspecte. [MENACE : Boot-kits, persistance pré-OS, rootkits UEFI]"
        HtmlAnchor = "bcd"
        Script     = {
    ...
    "07" = @{
        Name = "Stratégie sécurité"
        ShortName  = "SECEDIT"
        Category   = "Sécurité"
        Conseil    = "[CRITIQUE] Vérifier : MinimumPasswordLength >= 12, PasswordComplexity = 1, MaximumPasswordAge <= 90j, LockoutBadCount <= 5 (protection brute-force), LockoutDuration >= 15 min. PasswordComplexity = 0 ou LockoutBadCount = 0 (pas de verrouillage) expose au brute-force local. Vérifier que l'audit est activé (AuditLogonEvents, AuditObjectAccess) pour la forensique. [MENACE : brute-force local, pass-the-hash, absence de traces forensiques]"
        HtmlAnchor = "polsec"
        Script     = {
            Write-SectionHeader "POLITIQUE DE SÉCURITÉ (secedit)"
    ...
    "08" = @{
        ...
        ShortName  = "USERS"
        Category   = "Comptes"
        Conseil    = "[CRITIQUE] Le compte Administrateur doit être renommé et désactivé s'il est inutilisé : c'est la cible par défaut des outils d'attaque. Vérifier que chaque compte a un mot de passe fort. Les comptes inactifs doivent être désactivés. [MENACE : brute-force, mouvement latéral, élévation de privilèges]"
        HtmlAnchor = "licpt"
        Script     = {
    ...
    "09" = @{
        ...
        ShortName  = "GROUPS"
        Category   = "Comptes"
        Conseil    = "[IMPORTANT] Le groupe Administrateurs ne doit contenir que des comptes strictement nécessaires. Vérifier les comptes inconnus ou ajoutés récemment. Les comptes de service ne doivent PAS être dans le groupe Administrateurs. [MENACE : élévation de privilèges, persistance]"
        HtmlAnchor = "ligrp"
        Script     = {
    ...
    "11" = @{
        ...
        ShortName  = "USB"
        Category   = "Système"
        Conseil    = "[IMPORTANT] L'historique USB révèle tous les périphériques connectés depuis l'installation. Vérifier les clés USB inconnues, les adaptateurs réseau USB (exfiltration), rubber ducky / BadUSB (émulation clavier HID pour injection de commandes). Dans les environnements sensibles, la GPO doit bloquer les périphériques USB non autorisés. Chaque entrée inconnue doit être justifiée par l'utilisateur. [MENACE : BadUSB, exfiltration physique, introduction de malware, injection HID]"
        HtmlAnchor = "usb"
        Script     = {
            ...
            if ($result) {
                ...
                Write-Host "  Aucun périphérique USB trouvé dans le registre." -ForegroundColor DarkGray
    ...
    "13" = @{
        ...
        HtmlAnchor = "cfrzo"
        Script     = {
            Write-SectionHeader "CONFIGURATION RÉSEAU (ipconfig /all)"
    ...
    "14" = @{
        ...
        ShortName  = "NETSTAT"
        Category   = "Réseau"
        Conseil    = "[CRITIQUE] Vérifier les connexions ESTABLISHED vers des IP externes inconnues : possible communication C2 depuis un malware. Les ports LISTENING inhabituels (>1024 sur un poste) peuvent indiquer un RAT ou une backdoor. Vérifier les processus associés aux ports : svchost.exe sur un port non standard est suspect. Des connexions répétées vers la même IP en TIME_WAIT peuvent signaler un beaconing C2. [MENACE : RAT, C2, backdoor, beaconing, exfiltration réseau]"
        HtmlAnchor = "statrzo"
        Script     = {
    ...
    "16" = @{
        ...
        Script     = {
            ...
            if (($result -join "") -match "ERROR|Erreur") {
                Write-Host "  Aucune configuration WSUS trouvée (mise à jour directe Microsoft)." -ForegroundColor Yellow
            } else {
                $result | ForEach-Object { Write-Host "  $_" -ForegroundColor Cyan }
            }
            return ($result -join "`n")
    ...
    "17" = @{
        ...
        ShortName  = "HOTFIX"
        Category   = "MAJ"
        Conseil    = "[CRITIQUE] Vérifier la date de la dernière mise à jour : plus de 30 jours sans correctif est risqué, plus de 90 jours est critique. Rechercher les CVE critiques non corrigées : EternalBlue (MS17-010), PrintNightmare (CVE-2021-34527), Log4Shell, CVE-2022-30190. Comparer le build Windows avec les versions Microsoft pour identifier les correctifs manquants. Des KB de sécurité manquantes sont un fort indicateur de surface d'attaque ouverte. [MENACE : exploitation de CVE connues, ransomwares, vers de propagation]"
        HtmlAnchor = "maj"
        Script     = {
    ...
    "22" = @{
        ...
        ShortName  = "ARP"
        Category   = "Réseau"
        Conseil    = "[IMPORTANT] Une table ARP avec des adresses MAC dupliquées (deux IP avec le même MAC ou inversement) peut indiquer une attaque ARP poisoning active (man-in-the-middle). [MENACE : ARP spoofing, MITM, interception de credentials]"
        HtmlAnchor = "arp"
        Script     = {
    ...
    "23" = @{
        ...
        ShortName  = "HOSTS"
        Category   = "Réseau"
        Conseil    = "[CRITIQUE] Le fichier HOSTS est modifié par de nombreux malwaresools to: block AV updates (redirect windowsupdate.com to 127.0.0.1), redirect banking sites to phishing servers, prevent communication with remediation servers. Il ne doit contenir que la ligne localhoste (127.0.0.1 localhost) et optionally ::1 localhost. Any other entry must be justified. Compare hash between audits. [THREAT: AV evasion, local phishing, AV/update blocking, local DNS hijacking]"
        HtmlAnchor = "hosts"
        Script     = {
    ...
    "24" = @{
        ...
        Script     = {
            ...
            Write-Host "  En cours : $running  |  Arrêtés : $stopped" -ForegroundColor Yellow
            Write-Host ""
    ...
    "26" = @{
        ...
        HtmlAnchor = "infw"
        Script     = {
            Write-SectionHeader "RÈGLES PARE-FEU ENTRANTES (activées uniquement)"
            $rules = Get-NetFirewallRule -Direction Inbound -Enabled True |
                Select-Object DisplayName, Action, Protocol, Profile |
                Sort-Object Action
            Write-Host "  Total règles entrantes actives : $($rules.Count)" -ForegroundColor Yellow
    ...
    "27" = @{
        ...
        HtmlAnchor = "outfw"
        Script     = {
            Write-SectionHeader "RÈGLES PARE-FEU SORTANTES (activées uniquement)"
            $rules = Get-NetFirewallRule -Direction Outbound -Enabled True |
                Select-Object DisplayName, Action, Protocol, Profile |
                Sort-Object Action
            Write-Host "  Total règles sortantes actives : $($rules.Count)" -ForegroundColor Yellow
            $rules | ForEach-Object {
                $color = if ($_.Action -eq "Allow") { "Green" } else { "Red" }
                Write-Host ("  [{0,-6}] {1}" -f $_.Action, $_.DisplayName) -ForegroundColor $color
            }
    ...
    "29" = @{
        ...
        Conseil    = "[IMPORTANT] Les tâches planifiées sont un vecteur de persistance massivement utilisé par les malwares. Vérifier les tâches avec des auteurs inconnus, lançant PowerShell/cmd avec des paramètres encodés (-enc, -EncodedCommand), ou avec des chemins suspects. [MENACE : persistance, mouvement latéral, exécution de payloads malveillants]"
        HtmlAnchor = "tasks"
        Script     = {
            Write-SectionHeader "TÂCHES PLANIFIÉES (actives)"
    ...
    "34" = @{
        Name = "Démarrage système"
        ShortName  = "STARTUP"
        Category   = "Persistance"
        Conseil    = "[CRITIQUE] Les clés Run/RunOnce sont le vecteur de persistance le plus courant des malwares. Vérifier chaque entrée : le chemin doit pointer vers un binaire connu et signé dans un dossier système. Toute entrée pointant vers AppData, Temp ou un dossier utilisateur inhabituel est très suspecte. [MENACE : persistance, dropper, keylogger, RAT]"
        HtmlAnchor = "logdem"
        Script     = {
            Write-SectionHeader "PROGRAMMES AU DÉMARRAGE (Run Keys)"
            $runKeys = @(
    ...
    "35" = @{
        ...
        ShortName  = "IPv6"
        Category   = "Réseau"
        Conseil    = "[INFO] Si IPv6 n'est pas utilisé dans l'organisation, le désactiver réduit la surface d'attaquesurface. Teredo or 6to4 IPv6 addresses can be used to tunnel traffic et bypass firewalls not filtering IPv6. SLAAC (IPv6 auto-addressing) can assign addresses without DHCP control, making traceability difficult. An unknown IPv6 neighbor in the neighbor table may indicate a scanner or pivot. [THREAT: IPv6 tunnel, firewall bypass, network scan, pivot via Teredo]"
        HtmlAnchor = "adip6"
        Script     = {
    ...
    "38" = @{
        Name = "Journal Sécurité"
        ShortName  = "EVT-SEC"
        Category   = "Forensique"
        Conseil    = "[CRITIQUE] Le journal Sécurité contient des événements d'authentification critiquesion events. ID 4625 (logon failure) in bulk = ongoing brute-force. ID 4648 (logon with explicit credentials) = potential pass-the-h4sh. ID 4720/4732 (account creation / group addition) = suspicious persistence. ID 4698/4702 (scheduled task creation) = persistence. ID 4688 (process creation) if auditing enabled = execution traceability. [THREAT: Brute-force, pass-the-h4sh, suspicious account creation, privilege escalation]"
        HtmlAnchor = "evtsec"
        Script     = {
            Write-SectionHeader "JOURNAL SÉCURITÉ (50 derniers événements)"
            $output = @()
    ...
    "40" = @{
        ...
        Script     = {
            ...
            if (Test-Path $rdpKey) {
                ...
                Write-Host "  Aucun historique de connexion client RDP trouvé." -ForegroundColor DarkGray
    ...
    "41" = @{
        ...
        Script     = {
            ...
            if (-not $found) {
                Write-Host "  Historique PowerShell introuvable (PSReadLine non installé ou effacé)." -ForegroundColor Yellow
    ...
    "42" = @{
        ...
        Conseil    = "[CRITIQUE] L'UAC désactivé supprime les confirmations pour les opérations privilégiées, permettant à tout processus d'obtenir des droits SYSTEM silencieusement. Les attaquants désactivent systématiquement l'UAC après avoir compromis un compte admin. [MENACE : élévation de privilèges silencieuse, accès SYSTEM sans confirmation]"
        HtmlAnchor = "uac"
        Script     = {
            ...
            $uacLabels = @{
                "EnableLUA"                  = @{0="[RISQUE] UAC désactivé"; 1="[OK] UAC active"}
                "ConsentPromptBehaviorAdmin" = @{0="[RISQUE] Elevation silencieuse"; 1="Credential si non-admin"; 2="[OK] Demete de confirmation"; 5="[OK] Demete confirmation bureau securise"}
    ...
    "44" = @{
        ...
        Script     = {
            ...
            $label = if ($runAsPPL -eq 1) { "[PROTÉGÉ] LSA RunAsPPL actif" } else { "[RISQUE] LSA RunAsPPL désactivé - lsass lisible" }
            ...
            Write-Host ""
            Write-Host "  -- WDigest (mots de passe en clair en mémoire) --" -ForegroundColor Yellow
            ...
            $wdLabel = if ($useLogonCred -eq 1) { "[CRITIQUE] WDigest actif : mots de passe en clair en memoire !" } else { "[OK] WDigest désactivé (recommandé)" }
    ...
    "46" = @{
        ...
        Script     = {
            ...
            if ($modules) {
                ...
                Write-Host "  Aucun module non-système installé." -ForegroundColor DarkGray
...
function Compute-SecurityScore {
    ...
    if ($Results.ContainsKey("01")) {
        $s=10; $i=@()
        if (Has "01" "(?i)bootdebug\s+yes|testsigning\s+yes") { $s-=4; $i+="Mode debug/test actif (exécution de code non signé)" }
        if (Has "01" "(?i)recoveryenabled\s+no")               { $s-=2; $i+="Récupération désactivée" }
        AddDomain "Gestionnaire de démarrage (BCD)" "bcd" $s $i 2
    }
    ...
    if ($Results.ContainsKey("02")) {
        ...
        AddDomain "Système d'exploitation" "infos" $s $i 3
    }
    ...
    if ($Results.ContainsKey("03")) {
        ...
        AddDomain "Variables d'environnement" "infenv" $s $i 1
    }
    ...
    if ($Results.ContainsKey("05")) {
        ...
        if ($suspProc.Count -gt 0) { $s-=5; $i+="$($suspProc.Count) processus tournant depuis TEMP/APPDATA/PUBLIC détecté(s)" }
    ...
    if ($Results.ContainsKey("06")) {
        ...
        elseif (Has "06" "(?i)Bypass")   { $s-=5; $i+="ExecutionPolicy Bypass détecté" }
        elseif (Has "06" "(?i)Undefined") { $s-=2; $i+="ExecutionPolicy not defined on one or more scopes" }
        AddDomain "Politique PowerShell" "polpws" $s $i 2
    }
    ...
    if ($Results.ContainsKey("07")) {
        ...
        if ($r -match "MinimumPasswordLength\s*=\s*([0-9]+)") {
            $pl=[int]$Matches[1]
            if ($pl -lt 8)  { $s-=4; $i+="Mot de passe min $pl car. (recommandé >= 12)" }
            elseif ($pl -lt 12) { $s-=2; $i+="Mot de passe min $pl chars (recommande >= 12)" }
        }
        if ($r -match "PasswordComplexity\s*=\s*0") { $s-=3; $i+="Complexité des mots de passe désactivée" }
        if ($r -match "LockoutBadCount\s*=\s*0")    { $s-=3; $i+="Aucun verrouillage après échecs (brute-force possible)" }
        if ($r -match "MaximumPasswordAge\s*=\s*0")  { $s-=2; $i+="Expiration du mot de passe désactivée" }
        AddDomain "Stratégie de sécurité" "polsec" $s $i 3
    }
    ...
    if ($Results.ContainsKey("08")) {
        $s=10; $i=@()
        $r = ResultOf "08"
        if ($r -match "(?i)(Administrator|Administrateur).*Enabled\s*:\s*True") { $s-=3; $i+="Compte Administrateur intégré actif" }
        if ($r -match "(?i)(Guest|Invite).*Enabled\s*:\s*True")                  { $s-=3; $i+="Compte Invité actif" }
        if ($r -match "(?i)\[!MOT DE PASSE VIDE\]")                            { $s-=5; $i+="Compte(s) sans mot de passe détecté(s)" }
        AddDomain "Comptes locaux" "licpt" $s $i 3
    }
    ...
    if ($Results.ContainsKey("09")) {
        ...
        AddDomain "Groupes locaux" "ligrp" $s $i 2
    }
    ...
    if ($Results.ContainsKey("10")) {
        ...
        if ($totalLines -gt 8) { $s-=2; $i+="Nombreux partages détectés - vérifier les non documentés" }
        AddDomain "Partages réseau (SMB)" "shares" $s $i 3
    }
    ...
    if ($Results.ContainsKey("11")) {
        ...
        if ($usbCount -gt 10) { $s-=3; $i+="$usbCount périphérique(s) USB en historique - vérifier les inconnus" }
        ...
        AddDomain "Historique USB" "usb" $s $i 2
    }
    ...
    if ($Results.ContainsKey("12")) {
        ...
        AddDomain "Partitions / Disques" "part" $s $i 1
    }
    ...
    if ($Results.ContainsKey("13")) {
        ...
        if ($dnsLines | Where-Object { $_ -notmatch "(?i)8\.8\.8\.8|8\.8\.4\.4|1\.1\.1\.1|1\.0\.0\.1|192\.168\.|10\.|172\.(1[6-9]|2[0-9]|3[01])\." }) {
            $s-=2; $i+="Serveurs DNS non standard détectés - vérifier si autorisés"
        }
        if ($r -match "169\.254\.\d+\.\d+") { $s-=3; $i+="Adresse APIPA detectee (probleme DHCP)" }
        AddDomain "Configuration réseau" "cfrzo" $s $i 2
    }
    ...
    if ($Results.ContainsKey("14")) {
        ...
        if ($listening -gt 20) { $s-=2; $i+="$listening port(s) en écoute" }
        if (Has "14" "(?i):3389.*LISTENING|LISTENING.*:3389") { $s-=2; $i+="RDP (3389) exposé et en écoute" }
        if (Has "14" "(?i):445.*LISTENING|LISTENING.*:445")   { $s-=2; $i+="SMB (445) exposé et en écoute" }
        if (Has "14" "(?i):5985.*LISTENING|:5986.*LISTENING") { $s-=2; $i+="WinRM (5985/5986) exposé" }
        if (Has "14" "(?i):23\s|:21\s|:69\s")                 { $s-=3; $i+="Protocole non chiffré exposé (Telnet/FTP/TFTP)" }
        AddDomain "Ports et Connexions" "statrzo" $s $i 3
    }
    ...
    if ($Results.ContainsKey("16")) {
        ...
        AddDomain "Source MAJ (WSUS)" "wsus" $s $i 2
    }
    ...
    if ($Results.ContainsKey("17")) {
        ...
        if ($lines.Count -gt 0) {
            try {
                ...
                if ($daysSince -gt 90) { $s-=5; $i+="Dernière MAJ il y a $daysSince jours (critique)" }
                elseif ($daysSince -gt 30) { $s-=3; $i+="Dernière MAJ il y a $daysSince jours" }
                elseif ($daysSince -gt 15) { $s-=1; $i+="Dernière MAJ il y a $daysSince jours" }
            } catch {}
        } else { $s-=3; $i+="Impossible de déterminer la date de dernière MAJ" }
        AddDomain "Mises à jour" "maj" $s $i 4
    }
    ...
    if ($Results.ContainsKey("18")) {
        ...
        AddDomain "Synchronisation NTP" "ntp" $s $i 1
    }
    ...
    if ($Results.ContainsKey("19")) {
        ...
        # WEP = obsolete
        if ($r -match "(?i)Chiffrement\s*:\s*WEP|Cipher\s*:\s*WEP") { $s-=4; $i+="Chiffrement WEP obsolète détecté" }
        ...
        AddDomain "Configuration Wi-Fi" "wifi" $s $i 2
    }
    ...
    if ($Results.ContainsKey("22")) {
        ...
        if ($dupMacs.Count -gt 0) { $s-=5; $i+="Adresse(s) MAC dupliquée(s) - possible ARP spoofing" }
        AddDomain "Table ARP" "arp" $s $i 2
    }
    ...
    if ($Results.ContainsKey("23")) {
        ...
        if ($nonComment.Count -gt 0) {
            ...
            $i += "$($nonComment.Count) entrée(s) non standard dans le fichier HOSTS"
        }
        AddDomain "Fichier HOSTS" "hosts" $s $i 3
    }

    # -- 24 Services -----------------------------------------------------------
    if ($Results.ContainsKey("24")) {
        $s=10; $i=@()
        if (Has "24" "(?i)\[Running\].*Spooler|Spooler.*Running")               { $s-=2; $i+="Spooler actif (Print-Nightmare si non patché)" }
        if (Has "24" "(?i)\[Running\].*Telnet|Telnet.*Running")                  { $s-=4; $i+="Telnet actif (protocole non chiffré)" }
        if (Has "24" "(?i)\[Running\].*SNMP|SNMP.*Running")                      { $s-=2; $i+="SNMP actif - verify community string" }
        if (Has "24" "(?i)\[Running\].*RemoteRegistry|RemoteRegistry.*Running")  { $s-=3; $i+="RemoteRegistry actif (accès registre à distance)" }
        if (Has "24" "(?i)\[Running\].*WinRM|WinRM.*Running")                    { $s-=2; $i+="WinRM actif (PowerShell distant)" }
        AddDomain "Services Windows" "svc" $s $i 2
    }
    ...
    if ($Results.ContainsKey("25")) {
        ...
        if (Has "25" "(?i)State\s+OFF|Etat\s+INACTIF|Etat\s+Desactive|OFF") {
            $s-=6; $i+="Pare-feu désactivé sur au moins un profil"
        }
        if (NotHas "25" "(?i)Public.*ON|Public.*ACTIF") { $s-=3; $i+="Profil pare-feu Public non confirmé actif" }
        AddDomain "Pare-feu" "conffw" $s $i 4
    }
    ...
    if ($Results.ContainsKey("26")) {
        ...
        AddDomain "Règles FW entrantes" "infw" $s $i 2
    }

    # -- 28 Antivirus ----------------------------------------------------------
    if ($Results.ContainsKey("28")) {
        $s=10; $i=@()
        if (Has "28" "(?i)Real-time Protection Status\s*:\s*Disabled|Disabled") { $s-=5; $i+="Protection AV en temps réel désactivée" }
        if (Has "28" "(?i)Definition Status\s*:\s*Out of date|Out of date")      { $s-=4; $i+="Signatures AV obsolètes" }
        if (Has "28" "(?i)Inconnu")                                               { $s-=3; $i+="Antivirus non détecté ou erreur de lecture" }
    ...
    if ($Results.ContainsKey("29")) {
        ...
        if ($suspTasks.Count -gt 5) { $s-=4; $i+="$($suspTasks.Count) tâche(s) avec chemin/exe suspect(s)" }
        elseif ($suspTasks.Count -gt 2) { $s-=2; $i+="$($suspTasks.Count) tasks with system executables - verify" }
        AddDomain "Tâches planifiées" "tasks" $s $i 2
    }
    ...
    if ($Results.ContainsKey("30")) {
        $s=10; $i=@()
        $r = ResultOf "30"
        if ($r -match "(?i)AnyDesk|TeamViewer|RustDesk|UltraVNC|TightVNC|DWService") { $s-=3; $i+="Outil d'accès à distance détecté - vérifier si autorisé" }
        if ($r -match "(?i)Nmap|Wireshark|Metasploit|Mimikatz|Cobalt|Responder")     { $s-=5; $i+="Security/outil de test d'intrusion détecté" }
        if ($r -match "(?i)xmrig|nicehash|cgminer|bfgminer")                         { $s-=6; $i+="Logiciel de minage de cryptomonnaie détecté" }
        if ($r -match "(?i)Tor Browser|Tor-Browser")                                  { $s-=2; $i+="Navigateur Tor détecté" }
        AddDomain "Logiciels installés" "logs" $s $i 3
    }
    ...
    if ($Results.ContainsKey("31")) {
        ...
        if ($r -match "(?i)RT2870.*invalid|Service.*invalide.*0 non valide") { $s-=1; $i+="Erreur(s) de service répétée(s) détectée(s)" }
        AddDomain "Journal Système" "syst" $s $i 1
    }
    ...
    if ($Results.ContainsKey("32")) {
        ...
        if ($r -match "(?i)Application Error.*\.exe") { $s-=2; $i+="Crash applicatif détecté (vérifier le processus)" }
        AddDomain "Journal Application" "evtapp" $s $i 1
    }
    ...
    if ($Results.ContainsKey("33")) {
        ...
        if (NotHas "33" "(?i)Strategie|Policy|GPO") { $s-=2; $i+="Aucune GPO de sécurité détectée" }
        AddDomain "Stratégie de groupe" "rsop" $s $i 2
    }
    ...
    if ($Results.ContainsKey("34")) {
        ...
        if ($suspPaths.Count -gt 0) { $s-=5; $i+="$($suspPaths.Count) entrée(s) de démarrage avec chemin suspect" }
        elseif ($nonEmpty.Count -gt 8) { $s-=2; $i+="$($nonEmpty.Count) programme(s) au démarrage - vérifier les inconnus" }
        AddDomain "Démarrage système" "logdem" $s $i 3
    }
    ...
    if ($Results.ContainsKey("36")) {
        ...
        elseif ($r -match "(?i)Protection\s*:\s*Off|ProtectionStatus.*Off") { $s-=5; $i+="BitLocker désactivé sur au moins un volume" }
        elseif ($r -match "(?i)Chiffrement.*0%|EncryptionPercentage.*0") { $s-=4; $i+="Volume non chiffré détecté" }
        AddDomain "Chiffrement BitLocker" "bitlocker" $s $i 3
    }
    ...
    if ($Results.ContainsKey("37")) {
        ...
        AddDomain "Certificats système" "certs" $s $i 2
    }
    ...
    if ($Results.ContainsKey("38")) {
        ...
        if ($fails -gt 10)   { $s-=4; $i+="$fails échec(s) de connexion (4625) - possible brute-force" }
        elseif ($fails -gt 3){ $s-=2; $i+="$fails recent échec(s) de connexion" }
        if ($explicit -gt 5) { $s-=3; $i+="$explicit connexion(s) explicite(s) (4648) - possible pass-the-hash" }
        if ($newAcct -gt 0)  { $s-=4; $i+="Création/modification de compte ou groupe détectée (4720/4732)" }
        if ($newTask -gt 0)  { $s-=3; $i+="Création de tâche planifiée détectée dans les journaux (4698/4702)" }
        if ($r -match "(?i)Acces refuse|Access denied") { $s-=1; $i+="Access denied to Security log - insufficient rights" }
        AddDomain "Journal Sécurité" "evtsec" $s $i 4
    }
    ...
    if ($Results.ContainsKey("39")) {
        ...
        if ($suspFiles.Count -gt 5)  { $s-=5; $i+="$($suspFiles.Count) exécutable(s) suspect(s) modifié(s) dans les 72h" }
        ...
        AddDomain "Fichiers récents (72h)" "recentfiles" $s $i 3
    }
    ...
    if ($Results.ContainsKey("40")) {
        ...
        if ($r -match "(?i)Serveur\s*:\s*\d+\.\d+\.\d+\.\d+") {
            ...
            $s-=2; $i+="Connexion RDP vers IP directe dans l'historique (suspect)"
        }
        AddDomain "Historique RDP" "rdphist" $s $i 2
    }
    ...
    if ($Results.ContainsKey("41")) {
        $s=10; $i=@()
        $r = ResultOf "41"
        if ($r -match "(?i)non disponible|non installe|efface") { $s-=1; $i+="Historique PS absent ou effacé (possible nettoyage)" }
        $suspCmds = ($r -split "`n" | Where-Object { $_ -match "(?i)\[SUSPECT\]" })
        if ($suspCmds.Count -gt 3)  { $s-=5; $i+="$($suspCmds.Count) commande(s) PS suspecte(s) dans l'historique" }
        elseif ($suspCmds.Count -gt 0) { $s-=3; $i+="$($suspCmds.Count) commande(s) PS suspecte(s) détectée(s)" }
        AddDomain "Historique PowerShell" "pshist" $s $i 3
    }
    ...
    if ($Results.ContainsKey("42")) {
        $s=10; $i=@()
        $r = ResultOf "42"
        if ($r -match "EnableLUA.*0|\[RISQUE\] UAC desactive") { $s-=5; $i+="UAC désactivé - élévation silencieuse possible" }
        ...
        AddDomain "UAC et Privilèges" "uac" $s $i 3
    }
    ...
    if ($Results.ContainsKey("44")) {
        $s=10; $i=@()
        $r = ResultOf "44"
        if ($r -match "(?i)\[RISQUE\] LSA RunAsPPL desactive|RunAsPPL.*0") { $s-=4; $i+="LSA RunAsPPL désactivé - lsass lisible en mémoire" }
        if ($r -match "(?i)\[CRITICAL\] WDigest actif|UseLogonCredential.*1")  { $s-=5; $i+="WDigest actif - mots de passe en clair en mémoire !" }
        if ($r -match "EnableVirtualizationBasedSecurity.*0") { $s-=2; $i+="Virtualization Based Security désactivé" }
        AddDomain "Protection LSA / Cred. Guard" "lsaprot" $s $i 4
    }
    ...
    if ($Results.ContainsKey("45")) {
        ...
        if ($r -match "(?i)No connection|(?i)Aucune connexion") { }  # OK
            ...
            if ($suspProcs.Count -gt 0) { $s-=5; $i+="Connexion(s) externe(s) via processus suspect(s) (PS/cmd/mshta)" }
        }
        AddDomain "Connexions externes" "netgeo" $s $i 3
    }
    ...
    if ($Results.ContainsKey("46")) {
        $s=10; $i=@()
        $r = ResultOf "46"
        if ($r -match "(?i)Script Block Logging.*INACTIF") { $s-=2; $i+="PS Script Block Logging désactivé (détection réduite)" }
        if ($r -match "(?i)Module Logging.*INACTIF")       { $s-=1; $i+="PS Module Logging désactivé" }
        if ($r -match "(?i)Transcription.*INACTIF")        { $s-=1; $i+="PS Transcription désactivée" }
        $suspMods = ($r -split "`n" | Where-Object { $_ -match "(?i)appdata|temp|users\\[^\\]+\\documents" })
        if ($suspMods.Count -gt 0) { $s-=3; $i+="$($suspMods.Count) module(s) PS dans dossier utilisateur (chemin suspect)" }
        AddDomain "Modules PS / Journalisation" "psmodules" $s $i 2
    }
    ...
    if ($Results.ContainsKey("04")) {
        ...
        if ($r -match "(?i)WINWORD|EXCEL|POWERPNT|OUTLOOK") {
            if ($r -match "(?i)cmd\.exe|powershell") { $s-=5; $i+="cmd/PowerShell enfant de Word/Excel/Outlook (macro suspecte)" }
        }
        # svchost avec parent inhabituel
        if ($r -match "(?i)svchost.*ppid=(?!4|8|688|772|804|824)") { $s-=3; $i+="svchost.exe avec parent inhabituel détecté" }
        AddDomain "Arbre des processus" "arbps" $s $i 3
    }
    ...
    if ($Results.ContainsKey("27")) {
        ...
        AddDomain "Règles FW sortantes" "outfw" $s $i 1
    }
...
function Generate-MitreMatrix {
    ...
    for ($row = 0; $row -lt $maxRows; $row++) {
        ...
        foreach ($tac in $tactics.Keys) {
            ...
            if ($row -lt $tacTechs.Count) {
                ...
                $tip  = switch ($tlvl) { 3 {"CRITIQUE - activé par des vulnérabilités détectées"} 2 {"HIGH - favorable conditions"} 1 {"POSSIBLE - partial conditions"} default {"Not enabled"} }
    ...
    $flagLabels = @{
        ...
        AdminActive   = "Compte Admin intégré actif"
        NoPassword    = "Account with no password"
        WDigest       = "WDigest actif (mots de passe en mémoire)"
        NoLSAPPL      = "LSA RunAsPPL désactivé"
        ...
        SMBOpen       = "SMB exposé (port 445)"
        WinRMOpen     = "WinRM expose (5985/5986)"
        AdminShares   = "Partages admin actifs"
        FWDisabled    = "Pare-feu désactivé"
        AVDisabled    = "AV désactivé/obsolète"
    ...
    foreach ($fk in ($F.Keys | Sort-Object)) {
        if ($F[$fk] -eq $true) {
            ...
            if ($anc) {
                $activeFlags += "<a href='#$anc' class='flag-item flag-link' onclick='event.preventDefault();navToAnchor(`"$anc`")' title='Aller à la section'>$lbl <span class='flag-arrow'>&#8594;</span></a>`n"
            } else {
...
<section id='mitre-matrix' class='audit-section mitre-section'>
  <div class='section-header'>
    <h2>Matrice MITRE ATT&amp;CK Windows</h2>
  ...
  <div class='mitre-body'>
    ...
    <div class='mitre-flags'>
      <div class='flags-title'>Vulnérabilités détectées contribuant to the matrix:</div>
    ...
    <div class='mitre-footer'>
      Based on MITRE ATT&amp;CK v14 Enterprise / Windows. Cliquez sur a technique to open the ATT&amp;CK page.
...
function Generate-RemediationPlan {
    ...
    if (Has "08" "(?i)(\[!MOT DE PASSE VIDE\]|\[!EMPTY PASSWORD\])") {
        Add-Action 1 "Comptes" "Définir un mot de passe sur tous les comptes actifs" `
            "Les comptes sans mot de passe permettent un accès immédiat sans aucune identification." `
    ...
    if (Has "08" "(?i)(Administrator|Administrateur).*Enabled\s*:\s*True") {
        Add-Action 1 "Comptes" "Renommer et désactiver le compte Administrateur intégré" `
            "Le compte Administrateur intégré est la cible principale des attaques par brute-force." `
...
Disable-LocalUser -Name "Administrator"' `
    ...
    if (Has "08" "(?i)(Guest|Invite).*Enabled\s*:\s*True") {
        Add-Action 1 "Comptes" "Désactiver le compte Invité/Guest" `
            "Le compte Invité permet un accès authentifié sans identifiants connus." `
    ...
    if (Has "07" "LockoutBadCount\s*=\s*0") {
        Add-Action 1 "Comptes" "Activer le verrouillage de compte après des échecs" `
    ...
    if (Has "07" "PasswordComplexity\s*=\s*0") {
        Add-Action 1 "Comptes" "Activer la complexité des mots de passe" `
            "Sans complexité, les mots de passe simples (123456, prénom) sont courants." `
            'secedit /export /cfg C:\temp\secpol.cfg
# Définir PasswordComplexity = 1 dans le fichier
secedit /configure /db secedit.sdb /cfg C:\temp\secpol.cfg /overwrite' `
    ...
    if (Has "07" "MinimumPasswordLength\s*=\s*[0-9]\b") {
        ...
        if ($pwlen -lt 12) {
            Add-Action 2 "Comptes" "Augmenter la longueur minimale des mots de passe à 12" `
    ...
    if (Has "25" "(?i)State\s+OFF|Etat\s+INACTIF|OFF") {
        Add-Action 1 "Pare-feu" "Réactiver le pare-feu Windows sur tous les profils" `
            "Un pare-feu désactivé laisse tous les ports exposés sans filtrage." `
    ...
    if (Has "14" "(?i):3389.*LISTENING|LISTENING.*:3389") {
        Add-Action 1 "Pare-feu" "Restreindre l'accès RDP aux IP autorisées uniquement" `
...
New-NetFirewallRule -DisplayName "RDP - Block all" -Direction Inbound -Protocol TCP -LocalPort 3389 -Action Block' `
    ...
    if (Has "14" "(?i):445.*LISTENING|LISTENING.*:445") {
        Add-Action 1 "Pare-feu" "Bloquer le port SMB (445) en entrée depuis Internet" `
            "Le SMB exposé sur Internet permet l'exploitation de vulnérabilités critiques (EternalBlue, etc.)." `
    ...
    if (Has "14" "(?i):5985.*LISTENING|:5986.*LISTENING") {
        Add-Action 2 "Pare-feu" "Restreindre WinRM aux IP d'administration uniquement" `
            "WinRM permet l'exécution de commandes PowerShell à distance." `
    ...
    if ($Results.ContainsKey("17")) {
        ...
        if ($daysSince -gt 30) {
            Add-Action 1 "MAJ" "Installer les mises à jour Windows en attente ($daysSince jours de retard)" `
            "Chaque jour sans mise à jour expose le système aux CVE publiques et aux outils d'exploitation automatisés." `
...
Get-WindowsUpdate -Install -AcceptAll -AutoReboot' `
    ...
    if (Has "44" "(?i)RunAsPPL.*0|\[RISQUE\] LSA") {
        Add-Action 1 "Identifiants" "Activer LSA Protected Process Light (RunAsPPL)" `
...
# Redemarrage requis' `
    ...
    if (Has "44" "(?i)UseLogonCredential.*1|\[CRITICAL\] WDigest") {
        Add-Action 1 "Identifiants" "Désactiver WDigest pour éliminer les mots de passe en clair en mémoire" `
            "WDigest force Windows à conserver les credentials en clair dans lsass (héritage de Windows XP)." `
    ...
    if (Has "44" "EnableVirtualizationBasedSecurity.*0") {
        Add-Action 2 "Identifiants" "Activer Credential Guard (VBS)" `
            "Credential Guard isole les credentials dans un environnement virtualisé inaccessible depuis l'OS." `
...
reg add "HKLM\SYSTEM\CurrentControlSet\Control\Lsa" /v LsaCfgFlags /t REG_DWORD /d 1 /f' `
    ...
    if (Has "42" "(?i)EnableLUA.*0|\[RISQUE\] UAC desactive") {
        Add-Action 1 "Privilèges" "Réactiver l'UAC (User Account Control)" `
            "L'UAC désactivé permet à tout processus d'obtenir les privilèges SYSTEM sans confirmation." `
...
# Redemarrage requis' `
    ...
    if (Has "42" "(?i)ConsentPromptBehaviorAdmin.*0") {
        Add-Action 1 "Privilèges" "Configurer l'UAC pour demander confirmation (pas d'élévation silencieuse)" `
            "L'élévation silencieuse permet aux programmes malveillants de s'élever sans alerte utilisateur." `
    ...
    if (Has "36" "(?i)desactive|non configure|Off") {
        Add-Action 1 "Chiffrement" "Activer BitLocker sur le volume système" `
            "Un disque non chiffré peut être entièrement lu s'il est volé (démarrage USB, extraction disque)." `
    ...
    if (Has "28" "(?i)Disabled|Out of date") {
        Add-Action 1 "AV/EDR" "Réactiver et mettre à jour l'antivirus" `
            "Un AV désactivé ou obsolète n'offre aucune protection contre les menaces récentes." `
...
Update-MpSignature' `
    ...
    if (Has "06" "(?i)Unrestricted|Bypass") {
        Add-Action 1 "PowerShell" "Restreindre la politique d'exécution PowerShell" `
            "Unrestricted/Bypass permet à tout script PS de s'exécuter, vecteur majeur des attaques fileless." `
...
Set-ExecutionPolicy AllSigned -Scope CurrentUser -Force' `
    ...
    if (Has "46" "(?i)Script Block Logging.*INACTIF") {
        Add-Action 2 "PowerShell" "Activer le Script Block Logging PowerShell" `
            "Le Script Block Logging enregistre tout le code PS exécuté, y compris les commandes décodées." `
    ...
    if (Has "46" "(?i)Module Logging.*INACTIF") {
        Add-Action 3 "PowerShell" "Activer le Module Logging PowerShell" `
            "Le Module Logging enregistre les appels de fonctions depuis tous les modules PS chargés." `
    ...
    if (Has "43" "(?i)AppLocker non configure|aucune regle.*Exe") {
        Add-Action 2 "Contrôle d'exécution" "Configurer AppLocker pour restreindre l'exécution" `
            "AppLocker bloque l'execution depuis TEMP, APPDATA et les dossiers utilisateurs." `
            '# Créer les règles AppLocker par défaut
...
Set-Service AppIDSvc -StartupType Automatic' `
    ...
    if (Has "24" "(?i)\[Running\].*Telnet|Telnet.*Running") {
        Add-Action 1 "Services" "Désactiver le service Telnet" `
...
Set-Service -Name Telnet -StartupType Disabled' `
    ...
    if (Has "24" "(?i)\[Running\].*RemoteRegistry|RemoteRegistry.*Running") {
        Add-Action 2 "Services" "Désactiver le service RemoteRegistry" `
            "RemoteRegistry permet la lecture/écriture du registre à distance, vecteur de reconnaissance." `
...
Set-Service -Name RemoteRegistry -StartupType Disabled' `
    ...
    if (Has "24" "(?i)\[Running\].*SNMP|SNMP.*Running") {
        Add-Action 2 "Services" "Désactiver SNMP ou sécuriser la communauté" `
            "SNMP v1/v2 avec communauté 'public' expose les infos système à tout le réseau." `
...
Set-Service -Name SNMP -StartupType Disabled' `
    ...
    # ?? Partages ????????????????????????????????????????????????????????????
    if (Has "10" "(?i)ADMIN\`\$|C\`\$") {
        Add-Action 2 "Réseau" "Désactiver les partages administratifs cachés" `
            "Les partages admin (C`$, ADMIN`$) sont exploités par les ransomwares pour se propager." `
...
net share ADMIN$ /delete' `
    ...
    if (Has "23" "(?i)[^#\n]+\d+\.\d+\.\d+\.\d+") {
        ...
        if ($hostsLines.Count -gt 0) {
            Add-Action 1 "Réseau" "Nettoyer le fichier HOSTS des entrées non autorisées" `
                "$($hostsLines.Count) entrée(s) suspecte(s) trouvée(s). Ces entrées peuvent rediriger le trafic ou bloquer les mises à jour AV." `
...
notepad "$env:SystemRoot\System32\drivers\etc\hosts"' `
    ...
    # ?? Journalisation ??????????????????????????????????????????????????????
    if ($Results.ContainsKey("46")) {
        Add-Action 2 "Journalisation" "Augmenter la taille des journaux d'événements" `
        "Des journaux trop petits sont écrasés rapidement, détruisant les preuves forensiques." `
...
wevtutil sl Application /ms:524288000' `
    ...
    if (Has "46" "(?i)Transcription.*INACTIF") {
        Add-Action 3 "Journalisation" "Activer la transcription PowerShell" `
            "La transcription enregistre toutes les sessions PS dans des fichiers journaux horodatés." `
...
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\PowerShell\Transcription" /v OutputDirectory /t REG_SZ /d "C:\PSLogs" /f' `
    ...
    if (Has "35" "(?i)Teredo|6to4|ISATAP") {
        Add-Action 3 "Réseau" "Désactiver les tunnels IPv6 (Teredo, 6to4, ISATAP)" `
...
netsh interface isatap set state disabled' `
    ...
    if (Has "19" "(?i)WEP|Open.*Authentification|Authentication.*Open") {
        Add-Action 1 "Wi-Fi" "Supprimer les profils Wi-Fi avec chiffrement faible (WEP/ouvert)" `
            "Le WEP se casse en quelques minutes. Les réseaux ouverts exposent tout le trafic en clair." `
            '# Lister les profils :
...
netsh wlan delete profile name="<ProfileName>"' `
    ...
    # ?? ARP / Reseau ????????????????????????????????????????????????????????
    if (Has "22" "(?i)Doublon") {
        Add-Action 1 "Réseau" "Investiguer une anomalie ARP (possible ARP spoofing)" `
            "Une adresse MAC dupliquée dans la table ARP peut indiquer une attaque man-in-the-middle active." `
            '# Vérifier la table ARP :
arp -a
# Comparer avec la table de référence
# Identifier la machine avec l''adresse MAC dupliquee' `
    ...
    if (Has "21" "(?i)ProxyEnable.*1") {
        Add-Action 2 "Réseau" "Vérifier et valider la configuration du proxy" `
        "Un proxy configuré à l'insu de l'utilisateur permet l'interception de tout le trafic HTTP/HTTPS." `
            '# Vérifier le proxy configuré :
Get-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings" | Select ProxyServer,ProxyEnable
# Supprimer le proxy si non autorisé :
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings" -Name ProxyEnable -Value 0' `
    ...
    if (Has "34" "(?i)temp|appdata|public") {
        Add-Action 1 "Persistance" "Investiguer et nettoyer les entrées de démarrage suspectes" `
        "Les programmes se lançant depuis TEMP ou APPDATA sont caractéristiques des malwares." `
            '# Lister toutes les entrées Run :
Get-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run"
Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
# Supprimer une entrée suspecte :
Remove-ItemProperty -Path "HKCU:\...\Run" -Name "<EntryName>"' `
    ...
    if (Has "29" "(?i)\\\\Temp\\\\|\\\\AppData\\\\") {
        Add-Action 1 "Persistance" "Auditer et supprimer les tâches planifiées suspectes" `
        "Des tâches exécutant des scripts depuis TEMP ou APPDATA sont un signe de persistance suspecte." `
...
Unregister-ScheduledTask -TaskName "<TaskName>" -Confirm:$false' `
    ...
    # ?? Journal securite ????????????????????????????????????????????????????
    if (Has "38" "(?i)4625") {
        Add-Action 1 "Forensique" "Investiguer les échecs de connexion (EventID 4625)" `
            "Des échecs de connexion en masse indiquent un brute-force actif. Identifier l'IP source." `
...
Get-EventLog -LogName Security -InstanceId 4625 -Newest 50 | Select-Object TimeGenerated,Message | Format-List' `
    ...
    if (Has "38" "(?i)4720|4732") {
        Add-Action 1 "Forensique" "Investiguer la création/modification de comptes (4720/4732)" `
        "Un compte créé ou ajouté à un groupe sans autorisation indique une persistance." `
    ...
    # ?? WSUS HTTP ???????????????????????????????????????????????????????????
    if (Has "16" "(?i)http://") {
        Add-Action 1 "MAJ" "Sécuriser la communication WSUS en HTTPS" `
            "Un serveur WSUS en HTTP permet les attaques WSUS-pect : distribution de fausses mises à jour." `
    ...
    if (Has "41" "(?i)\[SUSPECT\]") {
        Add-Action 1 "Forensique" "Analyser les commandes PowerShell suspectes dans l'historique" `
        "Les commandes avec -enc, Download-String ou IEX sont caractéristiques d'une attaque fileless." `
            '# Consulter l historique PS complet :
Get-Content "$env:APPDATA\Microsoft\Windows\PowerShell\PSReadLine\ConsoleHost_history.txt"
# Analyser les événements Script Block (si actif) :
Get-WinEvent -LogName "Microsoft-Windows-PowerShell/Operational" | Where-Object Id -eq 4104' `
    ...
    function Build-ActionCards {
        ...
        foreach ($a in $list) {
            ...
            $refHtml = if ($a.Ref) { "<span class='rem-ref'>Réf. : $($a.Ref)</span>" } else { "" }
            $anchorHtml = if ($a.Anchor) { "<a href='#$($a.Anchor)' class='rem-section-link' onclick='event.preventDefault();navToAnchor(`"$($a.Anchor)`")'>&rarr; Aller à la section</a>" } else { "" }
            $html += @"
...
"@
    ...
    $cards1 = Build-ActionCards $p1 "p1" "CRITIQUE"
    $cards2 = Build-ActionCards $p2 "p2" "HAUTE"
    $cards3 = Build-ActionCards $p3 "p3" "MOYENNE"

    if (-not $cards1 -and -not $cards2 -and -not $cards3) {
        $cards1 = "<div class='rem-empty'>Aucune vulnérabilité majeure détectée automatiquement. Consultez les recommandations de chaque section.</div>"
    }
...
<section id='remediation' class='audit-section rem-section'>
  <div class='section-header'>
    ...
    <span class='badge'>REMÉDIATION</span>
    <span class='cat-pill'>$totalActions actions identifiées</span>
  </div>
  ...
  <div class='rem-body-wrap'>
    ...
    <div class='rem-summary'>
      <div class='rem-sum-card p1'>
        ...
        <span class='rem-sum-lbl'>Critique</span>
      </div>
      <div class='rem-sum-card p2'>
        <span class='rem-sum-num'>$($p2.Count)</span>
        <span class='rem-sum-lbl'>Priorité haute</span>
      </div>
      <div class='rem-sum-card p3'>
        <span class='rem-sum-num'>$($p3.Count)</span>
        <span class='rem-sum-lbl'>Priorité moyenne</span>
      </div>
      <div class='rem-disclaimer'>
        Les remédiations sont générées automatiquement depuis les constats détectés.
        Testez dans un environnement de validation avant le déploiement en production.
        Certaines actions nécessitent un redémarrage.
      </div>
    </div>

    $(if ($cards1) { "<div class='rem-group-title p1-title'>Actions critiques</div>$cards1" })
    $(if ($cards2) { "<div class='rem-group-title p2-title'>Priorité haute</div>$cards2" })
...
"@
    ...
    if (Has "10" "(?i)MAC.*dupliqu|(?i)ARP.*poison|(?i)anomalie ARP") {
        Add-Action 2 "Réseau" "Investiguer une anomalie ARP (possible ARP spoofing)" `
            "Une adresse MAC dupliquée dans la table ARP peut indiquer une attaque man-in-the-middle active." `
            '# Vérifier la table ARP :
arp -a
# Identifier les entrées avec MAC dupliqué et isoler la machine concernée' `
            "MITRE T1557.002" `
            "arp"
    }
    ...
    if (Has "13" "(?i)3389") {
        Add-Action 2 "Pare-feu" "Restreindre l'accès RDP aux IP autorisées uniquement" `
            "Le RDP exposé est le principal vecteur de brute-force et de ransomware." `
    ...
    if (Has "46" "(?i)expir") {
        Add-Action 3 "Chiffrement" "Renouveler les certificats expirés" `
            "Des certificats expirés peuvent bloquer des services et signalent un manque de surveillance PKI." `
...
function Build-HtmlReport {
    ...
    $scoreLabel = if ($globalScore -ge 7) { "Bon" } elseif ($globalScore -ge 5) { "Avertissement" } else { "Critique" }
    ...
    foreach ($d in $radarDomains) {
        ...
        $dlabel = if ($ds -ge 8) { "OK" } elseif ($ds -ge 5) { "Avertissement" } else { "Critique" }
        ...
        if ($d.Issues.Count -gt 0) {
            ...
            $issuesHtml = "<p class='domain-ok'>Aucun problème détecté</p>"
        }
        $domainCards += @"
...
"@
    ...
    foreach ($d in ($sortedDomains | Select-Object -First 10)) {
        $ds = $d.Score
        $dc = if ($ds -ge 8) { "#3fb950" } elseif ($ds -ge 5) { "#d29922" } else { "#f85149" }
        $dlabel = if ($ds -ge 8) { "Bon" } elseif ($ds -ge 5) { "Avertissement" } else { "Critique" }
        $dclass = if ($ds -ge 8) { "ok" } elseif ($ds -ge 5) { "warn" } else { "crit" }
        $radarScores += $ds
        $radarLabels += ($d.Label + "|" + $d.Anchor)
        $tipText = if ($d.Issues.Count -gt 0) { $d.Issues -join "; " } else { "Aucun problème détecté" }
...
<section id='dashboard' class='audit-section dashboard-section'>
  ...
  <div class='dashboard-body'>
    ...
    <div class='dash-top-row'>
      ...
      <div class='dash-left'>
        ...
        <div class='global-meta'>
          <div class='gm-title'>Score de sécurité global</div>
          <div class='gm-machine'>$computername</div>
          <div class='gm-date'>Audit : $date à $hour</div>
          <div class='gm-os'>$osCaption</div>
          <div class='gm-modules'>$modCount modules analysés</div>
          <div class='gm-legend'>
            <span class='leg crit'>0-4 Critique</span>
            <span class='leg warn'>5-7 Avertissement</span>
            <span class='leg ok'>8-10 Bon</span>
          </div>
        </div>
      </div>
      ...
      <div class='dash-radar'>
        <div class='radar-title'>Arbre de compétences sécurité</div>
      ...
      <div class='dash-right'>
        <div class='dl-title'>Répartition des domaines</div>
    ...
    <!-- Grille des domaines -->
    <div class='domains-title'>Analyse des domaines <span class='domains-hint'>-- Cliquez sur to go to the section</span></div>
...
"@
    ...
    foreach ($cat in ($allCats.Keys | Sort-Object)) {
        ...
        foreach ($k in $keysInCat) {
            ...
            $shortName = if ($name.Length -gt 20) { $name.Substring(0,19) + "…" } else { $name }
    ...
    foreach ($key in ($Results.Keys | Sort-Object)) {
        ...
        if ($mConseil) {
            ...
            if     ($mConseil -match "^\[CRITICAL\]")  { $cClass="critique";  $cLabel="CRITIQUE";  $cText=$mConseil -replace "^\[CRITICAL\]\s*","" }
...
"@
    ...
    # ---------------------------------------------------------------------------
    $logCatOrder = @("Système","Comptes","Processus","Réseau","Sécurité","Pare-feu","Persistance","Forensique","Journaux","Vulnérabilités","MAJ","Chiffrement","PowerShell")
    $logsCardsHtml = ""
...
<section id='logs-section' class='audit-section logs-section'>
  ...
  <div class='logs-body'>
    <p class='logs-intro'>Raw output from all executed audit modules. Cliquez sur une catégorie or card to expand.</p>
...
.dc-top { display: flex; justify-content: space-bandween; align-items: center; margin-bottom: 8px; }
...
.dns-domains-header {
  ...
  display: flex; align-items: center; justify-content: space-bandween;
  border-bottom: 1px solid var(--border);
}
...
#sidebar-top {
  ...
  display: flex; justify-content: space-bandween; align-items: center;
}
...
.global-score-wrap { display:none; }   /* hidden — replaced by dash-top-row */
...
<div id="header">
  ...
  <nav class="header-nav">
    ...
    <a class="hn-link hn-dashboard" href="#dashboard"        data-anchor="dashboard">Tableau de bord</a>
    <a class="hn-link hn-mitre"     href="#mitre-matrix"     data-anchor="mitre-matrix">Matrice MITRE</a>
    <a class="hn-link hn-rem"       href="#remediation"      data-anchor="remediation">Remédiation</a>
...
<!-- GUIDE PANEL -->
<div id="guide-panel" role="dialog" aria-label="Comment lire ce rapport">
  <div class="guide-inner">

    <div class="guide-header">
      <div>
        <h1><span class="g-icon">&#9432;</span> Comment lire ce rapport</h1>
        <p>Suivez ces étapes dans l'ordre pour tirer le meilleur de PowerAudit 3.2 — cliquez sur une étape pour y accéder directement.</p>
      </div>
      <button class="guide-close" id="guide-close-btn">&#10005; Fermer</button>
    </div>
    ...
    <div class="guide-steps">
      ...
      <a class="guide-step" style="--step-color:#58a6ff" href="#dashboard" onclick="navToAnchor('dashboard');toggleGuide()">
        <div class="guide-step-top">
          ...
          <div>
            <div class="guide-step-label">Tableau de bord de sécurité</div>
            <div class="guide-step-sub">Commencer ici</div>
          </div>
        </div>
        <div class="guide-step-desc">
          Votre point d'entrée. Le <strong>score global (0-10)</strong> vous donne donne une lecture instantanée de la posture de la machine.
          Le <strong>graphique radar</strong> montre quels domaines de sécurité sont faibles — survolez les nœuds pour les détails, cliquez pour accéder au plan de remédiation.
          La <strong>répartition des domaines</strong> liste tousus les domaines triés par score.
        </div>
        <div class="guide-step-actions">
          <span class="guide-action">Jauge de score global</span>
          <span class="guide-action">Graphique radar</span>
          <span class="guide-action">Cartes de domaines</span>
        </div>
      </a>
      ...
      <a class="guide-step" style="--step-color:#f85149" href="#mitre-matrix" onclick="navToAnchor('mitre-matrix');toggleGuide()">
        <div class="guide-step-top">
          ...
          <div>
            ...
            <div class="guide-step-sub" style="color:#f85149">Cartographie des menaces</div>
          </div>
        </div>
        <div class="guide-step-desc">
          See which <strong>techniques d'attaque</strong> are enabled by the detected vulnerabilities.
          <strong style="color:#f85149">Red</strong> = exposition critique (vulnerabilities confirmed),
          <strong style="color:#d29922">orange</strong> = risque élevé (favorable conditions),
          <strong style="color:#58a6ff">blue</strong> = possible (partial conditions).
          Cliquez sur une technique pour ouvrir la page ATT&amp;CK.
        </div>
        <div class="guide-step-actions">
          ...
          <span class="guide-action" style="background:rgba(210,153,34,.1);color:#d29922;border-color:rgba(210,153,34,.2)">Techniques haute priorité</span>
        </div>
      </a>
      ...
      <a class="guide-step" style="--step-color:#3fb950" href="#remediation" onclick="navToAnchor('remediation');toggleGuide()">
        <div class="guide-step-top">
          ...
          <div>
            <div class="guide-step-label">Remédiation</div>
            <div class="guide-step-sub" style="color:#3fb950">Corriger — par ordre de priorité</div>
          </div>
        </div>
        <div class="guide-step-desc">
          Cartes d'action priorisées, générées automatiquement depuis les résultats d'audit.
          Commencez par les <strong style="color:#f85149">Critical</strong> actions, puis les
          <strong style="color:#d29922">High</strong>, then <strong style="color:#58a6ff">Medium</strong>.
          Chaque carte comprend une <strong>commande PowerShell prête à l'exécution</strong> et un standard de référence (CIS, ANSSI, NIST).
          Le lien <em>Aller à la section</em> mène directement au log brut du module.
        </div>
        <div class="guide-step-actions">
          <span class="guide-action" style="background:rgba(248,81,73,.1);color:#f85149;border-color:rgba(248,81,73,.2)">Critique — agir immédiatement</span>
          <span class="guide-action" style="background:rgba(210,153,34,.1);color:#d29922;border-color:rgba(210,153,34,.2)">Haute — planifier rapidemand</span>
          <span class="guide-action" style="background:rgba(88,166,255,.1);color:#58a6ff;border-color:rgba(88,166,255,.2)">Moyenne — durcissement</span>
        </div>
      </a>
      ...
      <a class="guide-step" style="--step-color:#a371f7" href="#logs-section" onclick="navToAnchor('logs-section');toggleGuide()">
        <div class="guide-step-top">
          ...
          <div>
            <div class="guide-step-label">Journaux d'audit</div>
            <div class="guide-step-sub" style="color:#a371f7">Résultats bruts des modules</div>
          </div>
        </div>
        <div class="guide-step-desc">
          Tous les 28+ résultats de modules, organisés par catégorie et repliés par défaut.
          <strong>Cliquez sur une catégorie</strong> (System, Security, Network…) pour la développer,
          puis <strong>click a card</strong> pour voir le résultat brut et le conseil de sécurité.
          Les cartes avec <strong style="color:#f85149">titres rouge/orange</strong> have linked action de remédiations —
          click the footer banner to jump directly to them.
        </div>
        <div class="guide-step-actions">
          <span class="guide-action" style="background:rgba(163,113,247,.1);color:#a371f7;border-color:rgba(163,113,247,.2)">Groupes de catégories</span>
          <span class="guide-action" style="background:rgba(163,113,247,.1);color:#a371f7;border-color:rgba(163,113,247,.2)">Cartes repliables</span>
          <span class="guide-action" style="background:rgba(163,113,247,.1);color:#a371f7;border-color:rgba(163,113,247,.2)">Lien de remédiation</span>
        </div>
      </a>
      ...
      <div class="guide-step" style="--step-color:#d29922;cursor:default">
        <div class="guide-step-top">
          ...
          <div>
            <div class="guide-step-label">Navigation dans le menu</div>
            <div class="guide-step-sub" style="color:#d29922">Accéder partout instantanément</div>
          </div>
        </div>
        <div class="guide-step-desc">
          Le menu latéral gauche liste chaque module par catégorie.y.
          <strong>Cliquez sur une catégorie name</strong> pour replier/développer le groupe.
          <strong>Cliquez sur un nom de module</strong> pour accéder directement à sa carte de journal.
          Utilisez la <strong>barre de recherche</strong> (en haut à droite) pour filtrer les modules par nom.
          <strong>Points de navigation</strong> (petits cercles) indiquent le statut de lecture :
          <span style="color:#3fb950">●</span> ouvert, <span style="color:#8b949e">○</span> consulté.
        </div>
        <div class="guide-step-actions">
          <span class="guide-action" style="background:rgba(210,153,34,.1);color:#d29922;border-color:rgba(210,153,34,.2)">Tout développer / réduire</span>
          <span class="guide-action" style="background:rgba(210,153,34,.1);color:#d29922;border-color:rgba(210,153,34,.2)">Filtre de recherche</span>
          <span class="guide-action" style="background:rgba(210,153,34,.1);color:#d29922;border-color:rgba(210,153,34,.2)">Indicateurs de lecture</span>
        </div>
      </div>
      ...
      <div class="guide-step" style="--step-color:#3fb950;cursor:pointer" onclick="window.guideClose&&window.guideClose();window.toggleDelivery&&window.toggleDelivery()">
        <div class="guide-step-top">
          ...
          <div><div class="guide-step-label">REPORT</div><div class="guide-step-sub" style="color:#3fb950">Exporter et livrer</div></div>
        </div>
        <div class="guide-step-desc">
          Génère un rapport autonome : page de couverture avec le <strong>Skill Tree</strong>, tableau des domaines, aperçu MITRE,
          et chaque constat avec sa <strong>commande de remédiation</strong> et <strong>résultat brut du journal</strong>.
          La table des matières renvoie à chaque vulnérabilité.ity. Cliquez sur <strong>Exporter PDF</strong> pour imprimer.
        </div>
        <div class="guide-step-actions">
          ...
          <span class="guide-action" style="background:rgba(63,185,80,.1);color:#3fb950;border-color:rgba(63,185,80,.2)">Index des constats</span>
          <span class="guide-action" style="background:rgba(63,185,80,.1);color:#3fb950;border-color:rgba(63,185,80,.2)">Exporter PDF</span>
        </div>
      </div>

    </div><!-- /.guide-steps -->

    <div class="guide-features-title">Fonctionnalités supplémentaires</div>
    <div class="guide-features">
      ...
      <div class="guide-feat">
        ...
        <div class="guide-feat-body">
          ...
          <span>Cliquez sur the moon/sun button in the top-right corner. Your preference is remembered between sessions.</span>
        </div>
      </div>
      ...
      <div class="guide-feat">
        <span class="guide-feat-icon">&#8249;</span>
        <div class="guide-feat-body">
          <strong>Replier le menu</strong>
          <span>Cliquez sur the <em>‹</em> button on the sidebar edge to hide it et gain full-width reading space.</span>
        </div>
      </div>
      ...
      <div class="guide-feat">
        <span class="guide-feat-icon">&#8679;</span>
        <div class="guide-feat-body">
          <strong>Retour en haut</strong>
          <span>Le bouton <em>↑</em> (bas droite) apparaît lorsquehen you scroll down et returns you to the dashboard.</span>
        </div>
      </div>
      ...
      <div class="guide-feat">
        <span class="guide-feat-icon">&#9432;</span>
        <div class="guide-feat-body">
          <strong>Panneau de références</strong>
          <span>Cliquez sur <em>References</em> in the header to open a curated list of security standards (CIS, NIST, ANSSI, MITRE) in the sidebar.</span>
        </div>
      </div>
    ...
    <div class="guide-footer">
      PowerAudit 3.2 — BlueTeam — Apache 2.0 —
      Les résultats sont générés automatiquement ; validez toujours les constats avant d'agir en production.
    </div>
...
<div id="delivery-panel" role="dialog" aria-label="Rapport d'audit">
  <div class="delivery-inner">
    ...
    <div id="delivery-body"><div class="delivery-loading">&#9203; Construction en cours…</div></div>
  </div>
</div>
...
<div id="sidebar">
  <div id="sidebar-top">
    ...
    <div style="display:flex;gap:5px">
      <button id="expand-all">Tout développer</button>
      <button id="collapse-all">Tout réduire</button>
    </div>
  </div>
  <nav>
    <ul id="nav-list">
    <li><a class='nav-link dashboard-link' href='#dashboard' data-anchor='dashboard'>Tableau de bord</a></li>
    <li><a class='nav-link mitre-link' href='#mitre-matrix' data-anchor='mitre-matrix'>Matrice MITRE ATT&amp;CK</a></li>
    <li><a class='nav-link rem-link' href='#remediation' data-anchor='remediation'>Plan de remédiation</a></li>
    ...
    <li><a class='nav-link nav-logs' href='#logs-section' data-anchor='logs-section'>Journaux d'audit</a></li>
$menuItems
    </ul>
  </nav>
  ...
  <div id="sidebar-refs">
    <button id="refs-toggle" class="refs-toggle-btn">
      ...
      <span class="refs-title-text">Références de sécurité</span>
...
<button id="sidebar-collapse-btn" title="Replier le menu">&#8249;</button>
      ...
      if (isMobile()) {
        ...
        if (open) {
          ...
          if (collapseBtn) {
            ...
            collapseBtn.title      = 'Replier le menu';
          }
        } else {
  ...
  (function() {
    ...
    function draw() {
      ...
      for (var i=0; i<N; i++) {
        ...
        var name=domainNames[i]; if (name.length>10) name=name.substring(0,9)+'…';
    ...
    canvas.addEventListener('mousemove', function(e) {
      ...
      if (found>=0) {
        ...
        var lbl=s<=4?'Critique':s<=7?'Avertissement':'Bon';
        tooltip.innerHTML="<span class='rt-name'>"+domainNames[found]+"</span>"+
          "<span class='rt-score' style='color:"+nc+"'>"+s+" / 10 — "+lbl+"</span>"+
          "<span class='rt-issues' style='color:var(--dim);font-size:.68rem'>Cliquez sur → go to remediation</span>";
    ...
    document.querySelectorAll('.log-footer-check').forEach(function(div) {
      ...
      if (matches.length > 0) {
        ...
        div.innerHTML='<div class="log-footer-rem" style="cursor:pointer" onclick="goToRem(\''+catLow+'\')">'
          ...
          +'<span class="lf-text"><strong>'+matches.length+' action de rem\u00E9diation'+(matches.length>1?'s':'')+' \u2014 '+cat+'</strong>'
          +parts.join(' &nbsp;')+'</span>'
        ...
        div.innerHTML='<div class="log-footer-ok"><span class="lf-icon">&#10003;</span>'
          ...
          +'R\u00E9vision manuelle toujours recommand\u00E9e.</span></div>';
      }
    });
  })();
...
(function(){
  ...
  function getLog(anchor){
    ...
    var key=lines.filter(function(l){
      return /\[RISQUE\]|\[CRITIQUE\]|\[IMPORTANT\]|\[ALERT\]|\[WARN\]|\bRISK\b|d\u00E9tect\u00E9|d\u00E9tect\u00E9e|d\u00E9sactiv\u00E9|expos\u00E9|suspect|anormal|actif\b|bypass|vuln/i.test(l) && l.trim().length>2;
    });
  ...
  function buildPrio(prio,label,color,num){
    ...
    cards.forEach(function(card){
      ...
      html+='<div class="dr-finding" id="dr-finding-'+_fi+'"><div class="dr-finding-header">'
        ...
        +'</div>';
      if(logOut)html+='<div class="dr-log-block"><div class="dr-log-label">&#9888; Constat cl\u00E9 — '+(anchor||cat)+'</div><pre>'+esc(logOut)+'</pre></div>';
      html+='</div>'; _fi++;
    });
    return html+'</div>';
  }
  ...
  function buildDelivery(){
    ...
    var scoreEl=document.querySelector('.global-gauge text[font-size="32"]'),score=scoreEl?scoreEl.textContent:'—';
    ...
    if(src){try{var img=src.toDataURL('image/png');
      radarHtml='<div class="dr-cover-radar"><div class="dr-cover-radar-title">Arbre de comp\u00E9tences s\u00E9curit\u00E9</div><img src="'+img+'" class="dr-cover-radar-img" alt="Arbre de comp\u00E9tences"/></div>';
    }catch(e){}}
    // Domains
    var domRows=[];
    ...
    // MITRE — stats + TTP list
    var mitreHtml='<div class="dr-section" id="dr-mitre">'      +'<div class="dr-section-title"><span class="dr-section-num" style="background:#f85149">2</span>Aper\u00E7u MITRE ATT&amp;CK</div>'      +'<p class="dr-section-intro">Le framework MITRE ATT&amp;CK r\u00E9f\u00E9rence les techniques d\'attaque connues. Les techniques ci-dessous ont \u00E9t\u00E9 activ\u00E9es ou facilit\u00E9es par les vuln\u00E9rabilit\u00E9s d\u00E9tect\u00E9es sur cette machine. Cliquez sur un identifiant pour acc\u00E9der \u00E0 la fiche MITRE officielle.</p>'      +'<div class="dr-mitre-grid">';
    ...
    function buildTTPTable(ttps,color,label){
      ...
      var html='<div class="dr-ttp-group">'        +'<div class="dr-ttp-header" style="color:'+color+';border-color:'+color+'20">'        +'<span class="dr-ttp-badge" style="background:'+color+'20;border:1px solid '+color+'40;color:'+color+'">'+label+'</span>'        +' '+ttps.length+' technique'+(ttps.length>1?'s':'')+' identifi\u00E9e'+(ttps.length>1?'s':'')        +'</div><div class="dr-ttp-list">';
    ...
    if(hasTTP){
      ...
      mitreHtml+=buildTTPTable(ttpsByLevel.crit,'#f85149','CRITIQUE');
      mitreHtml+=buildTTPTable(ttpsByLevel.warn,'#d29922','\u00C9LEV\u00C9');
      mitreHtml+=buildTTPTable(ttpsByLevel.poss,'#58a6ff','POSSIBLE');
      mitreHtml+='</div>';
    } else {
      mitreHtml+='<p style="color:var(--dim);font-size:.85rem;padding:12px 0">Aucune technique activ\u00E9e automatiquement d\u00E9tect\u00E9e. Consultez les recommandations de chaque module.</p>';
    }
    mitreHtml+='</div>';
    // Counts + index
    ...
    // TOC
    var toc='<div class="dr-toc"><div class="dr-toc-title">Table des mati\u00E8res</div><div class="dr-toc-cols">'
      +'<div class="dr-toc-sections"><ol>'
      +'<li><a href="#" onclick="scrollDr(\'dr-cover\')">R\u00E9sum\u00E9 ex\u00E9cutif</a></li>'
      +'<li><a href="#" onclick="scrollDr(\'dr-mitre\')">Aper\u00E7u MITRE ATT&amp;CK</a></li>'
      ...
      +'</ol></div>'
      +'<div class="dr-toc-index"><div class="dr-toc-index-title">Index des vuln\u00E9rabilit\u00E9s</div><ol>';
    allF.forEach(function(f,i){
      var color=f.prio==='p1'?'#f85149':f.prio==='p2'?'#d29922':'#58a6ff';
      toc+='<li><a href="#" onclick="scrollDr(\'dr-finding-'+i+'\')" style="color:'+color+'">'+esc(f.cat)+' — '+esc(f.title)+'</a></li>';
    });
    toc+='</ol></div></div></div>';
    // Domain table
    var dt='<table class="dr-domain-table"><thead><tr><th>Domaine</th><th>Score</th><th>Constat</th></tr></thead><tbody>';
    ...
    body.innerHTML=
      ...
      +'<div class="cyber-header-right"><span class="cyber-tag">RAPPORT D’AUDIT</span><span class="cyber-tag cyber-tag-date">'+esc(date)+'</span></div>'
      +'</div>'
      ...
      +'<div class="cyber-score-label" style="color:'+scoreColor+'">Score de s\u00E9curit\u00E9 global</div>'
      +'</div></div>'
      ...
      +'<strong>Avertissement — Usage légal uniquement</strong><br>'
      +'Ce rapport est généré par PowerAudit 3.2, un outil de pré-audit de configuration Windows. '
      +'Il ne constitue pas un audit de sécurité exhaustif. '
      +'<strong>BoneFlag</strong> décline toute responsabilité concernant l’utilisation de ce script, des commandes générées, '
      +'ou des actions entreprises sur la base de ces résultats. '
      +'&#9888; L’utilisation est strictement limitée aux systèmes dont vous êtes l’administrateur légitime. '
      +'Toute utilisation non autorisée constitue une infraction pénale.'
      +'</div></div>'
      +'</div>'
      +toc
      +'<div class="dr-section" id="dr-summary">'      +'<div class="dr-section-title"><span class="dr-section-num">1</span>R\u00E9sum\u00E9 ex\u00E9cutif</div>'      +'<p class="dr-section-intro">Ce tableau r\u00E9sume la posture de s\u00E9curit\u00E9 globale de <strong>'+esc(machine)+'</strong> audit\u00E9e le <strong>'+esc(date)+'</strong>. Chaque domaine est scor\u00E9 de 0 (risque critique) \u00E0 10 (aucun probl\u00E8me). Les domaines en rouge n\u00E9cessitent une action imm\u00E9diate, ceux en orange m\u00E9ritent une planification \u00E0 court terme.</p>'      +dt+'</div>'
      +mitreHtml
      +'<div class="dr-section" id="dr-findings-intro">'      +'<p class="dr-section-intro">Les sections suivantes d\u00E9taillent chaque constat par ordre de priorit\u00E9, avec le contexte de la vuln\u00E9rabilit\u00E9, la commande de rem\u00E9diation pr\u00EAte \u00E0 l’emploi, et le r\u00E9sultat brut du module d’audit correspondant. Commencez par les constats <strong style="color:#f85149">critiques</strong>, puis traitez les priorit\u00E9s haute et moyenne.</p></div>'      +buildPrio('p1','Constats critiques &amp; rem\u00E9diation','#f85149','3')
      +buildPrio('p2','Constats haute priorit\u00E9 &amp; rem\u00E9diation','#d29922','4')
      +buildPrio('p3','Constats priorit\u00E9 moyenne &amp; rem\u00E9diation','#58a6ff','5');
  }
  ...
  window.exportDeliveryPDF=function(){
    ...
    function getLog(anchor){
      ...
      var lines=full.split('\n');
      var key=lines.filter(function(l){
        return /\[RISQUE\]|\[CRITIQUE\]|\[IMPORTANT\]|\[ALERT\]|d\u00E9tect\u00E9|d\u00E9tect\u00E9e|d\u00E9sactiv\u00E9|expos\u00E9|suspect|anormal|actif\b|bypass|vuln/i.test(l) && l.trim().length>2;
      });
    ...
    if(cvs){try{radarImg='<img class="radar-img" src="'+cvs.toDataURL('image/png')+'" alt="Arbre de comp\u00E9tences s\u00E9curit\u00E9"/>';}catch(er){}}
    ...
    document.querySelectorAll('.dl-item').forEach(function(item){
      ...
      var status=cls==='crit'?'Critique':cls==='warn'?'Avertissement':'Bon';
      ...
      cards.forEach(function(card,i){
        ...
        if(logOut){
          html+='<div class="log-block">'
            +'<div class="log-label">&#128196; R\u00E9sultat brut d’audit — '+e(anchor||cat)+'</div>'
    ...
    ['p1','p2','p3'].forEach(function(prio){
      document.querySelectorAll('.rem-card.'+prio).forEach(function(card){
        ...
        tocItems+='<li style="color:'+col+';padding:2px 0"><span style="color:'+dim+';font-weight:400">'+e(cat)+' — </span>'+e(title)+'</li>';
        fi++;
      });
    });
    ...
    var html='<!DOCTYPE html><html lang="en"><head>'
      +'<meta charset="UTF-8"><title>PowerAudit 3.2 — Security Report</title>'
    ...
    html+='<div class="toc"><h3>Table des mati\u00E8res</h3><ol>'
      +'<li>R\u00E9sum\u00E9 ex\u00E9cutif — Analyse des domaines</li>'
      +'<li>Aper\u00E7u MITRE ATT&amp;CK</li>'
      +(p1c?'<li>Constats critiques &amp; rem\u00E9diation ('+p1c+')</li>':'')
      +(p2c?'<li>Constats haute priorit\u00E9 &amp; rem\u00E9diation ('+p2c+')</li>':'')
      +(p3c?'<li>Constats priorit\u00E9 moyenne &amp; rem\u00E9diation ('+p3c+')</li>':'')
      +'</ol>'
      +(tocItems?'<h3 style="margin-top:16px">Index des vuln\u00E9rabilit\u00E9s</h3><ol>'+tocItems+'</ol>':'')
      +'</div>';

    // Section 1 — R\u00E9sum\u00E9 ex\u00E9cutif
    html+='<div class="section"><h2 style="color:'+teal+';border-color:'+teal+'">1. R\u00E9sum\u00E9 ex\u00E9cutif</h2>'
      +'<p class="section-intro">Le tableau suivant r\u00E9sume la posture de s\u00E9curit\u00E9 de <strong>'+esc(machine)+'</strong>, '
      +'audit\u00E9 le <strong>'+esc(date)+'</strong>. Chaque domaine est scor\u00E9 de 0 (risque critique) \u00E0 10 (aucun probl\u00E8me). '
      +'Les domaines scor\u00E9s 0–4 n\u00E9cessitent une action imm\u00E9diate.</p>'
      +'<table><thead><tr><th>Domaine</th><th>Score</th><th>Statut</th><th>Constat</th></tr></thead>'
      +'<tbody>'+domRows+'</tbody></table></div>';

    // Section 2 — MITRE
    html+='<div class="section"><h2 style="color:#f85149;border-color:#f85149">2. Aper\u00E7u MITRE ATT&amp;CK</h2>'
      +'<p class="section-intro">Le framework MITRE ATT&amp;CK r\u00E9f\u00E9rence les techniques d\'attaque connues. Les techniques list\u00E9es ci-dessous ont \u00E9t\u00E9 activ\u00E9es ou facilit\u00E9es par les vuln\u00E9rabilit\u00E9s d\u00E9tect\u00E9es sur cette machine. '
      +'<strong style="color:#f85149">Critique</strong> signifie que la technique est directement exploitable avec la configuration actuelle. '
    ...
    ['crit','warn','poss'].forEach(function(lvl){
      ...
      var lbl=lvl==='crit'?'CRITIQUE':lvl==='warn'?'\u00C9LEV\u00C9':'POSSIBLE';
    ...
    html+='<div class="section"><p class="section-intro" style="background:'+bg2+';border-left:3px solid #06b6d4;padding:10px 14px;border-radius:0 6px 6px 0;font-size:.86rem;color:'+dim+';line-height:1.7">Les sections suivantes d\u00E9taillent chaque constat par ordre de priorit\u00E9, avec la commande de rem\u00E9diation pr\u00EAte \u00E0 l’emploi et le r\u00E9sultat brut du module d’audit.</p></div>';
    html+=buildFindings('p1','3. Constats critiques &amp; rem\u00E9diation','#f85149');
    html+=buildFindings('p2','4. Constats haute priorit\u00E9 &amp; rem\u00E9diation','#d29922');
    html+=buildFindings('p3','5. Constats priorit\u00E9 moyenne &amp; rem\u00E9diation','#58a6ff');
...
function Show-MainMenu {
    ...
    Write-Host ""
    Write-Host "    [1]  Audit complet   -- tous les modules -- rapport HTML" -ForegroundColor Green
    Write-Host "    [2]  Audit sélectif  -- choisir les modules" -ForegroundColor Green
    Write-Host ""
    Write-Host "  INVESTIGATION" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "    [3]  Exécuter un module -- résultat dans le shell" -ForegroundColor Cyan
    Write-Host "    [4]  Ouvrir le dernier rapport HTML" -ForegroundColor Cyan
    Write-Host "    [5]  Résumé de la machine" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  ----------------------------------------------------------------------" -ForegroundColor DarkGray
    Write-Host ""
...
function Run-Menu2 {
    ...
    while ($true) {
        ...
        Write-Host "  |  MENU 2 -- SÉLECTION DE MODULES POUR RAPPORT PERSONNALISÉ  |" -ForegroundColor Cyan
        Write-Host "  +==================================================================+" -ForegroundColor Cyan
        Write-Host ""
        Write-Host "  Commandes : [numéro] cocher/décocher  |  A=tout  |  N=aucun  |  V=valider & générer  |  Q=retour" -ForegroundColor DarkGray
        Write-Host ("  " + ("-" * 90))
        Write-Host ""
        ...
        Write-Host ""
        Write-Host ("  Modules sélectionnés : {0}/{1}   (V=valider & générer rapport  |  Q=retour sans générer)" -f $Global:Menu2Selection.Count, $Global:AuditModules.Count) -ForegroundColor Yellow
        Write-Host ""
        ...
        elseif ($cmd -eq "V" -or $cmd -eq "v") {
            if ($Global:Menu2Selection.Count -eq 0) {
                Write-Host "  Aucun module sélectionné." -ForegroundColor Red
        ...
        elseif ($Global:AuditModules.Contains($cmd)) {
            if ($Global:Menu2Selection -contains $cmd) {
                ...
                Write-Host ("  [-] {0} retiré" -f $cmd) -ForegroundColor DarkGray
            } else {
                $Global:Menu2Selection += $cmd
                Write-Host ("  [+] {0} ajouté" -f $cmd) -ForegroundColor Green
            }
...
function Run-Menu3 {
    while ($true) {
        ...
        Write-Host ""
        Write-Host "  +==================================================================+" -ForegroundColor Cyan
        Write-Host "  |  MENU 3 -- EXÉCUTION INDIVIDUELLE (résultat dans le shell)  |" -ForegroundColor Cyan
        Write-Host "  +==================================================================+" -ForegroundColor Cyan
        Write-Host ""
        $currentCategory = ""
        foreach ($key in $Global:AuditModules.Keys) {
        ...
        $cmd = (Read-Host "  Numéro de module (Q=retour au menu principal)").Trim()
        ...
        elseif ($Global:AuditModules.Contains($cmd)) {
            ...
            Write-Host ("  Module terminé en {0:F1}s" -f $elapsed) -ForegroundColor Yellow
            Write-Host ("  " + ("=" * 68)) -ForegroundColor DarkCyan
            Write-Host ""
            Read-Host "  Appuyez sur Entrée pour revenir à la liste"
        }
        else {
...
function Run-Modules {
    ...
    Write-Host ""
    Write-Host "  Démarrage de l'audit -- $total module(s) sélectionné(s)" -ForegroundColor Yellow
    Write-Host ""
    ...
    foreach ($key in $sortedKeys) {
        ...
        try {
            $output      = & $m.Script
            $results[$key] = if ($output) { $output | Out-String } else { "(Aucun résultat)" }
        } catch {
            Write-Host "  [ERREUR module $key] $($_.Exception.Message)" -ForegroundColor Red
            $results[$key] = "[ERROR] $($_.Exception.Message)"
        }
        $elapsed = ((Get-Date) - $startTime).TotalSeconds
        Write-Host ("  OK {0} -- terminé en {1:F1}s" -f $m.Name, $elapsed) -ForegroundColor DarkGreen
    }

    Write-Host ""
    Write-ProgressBar -Current $total -Total $total -Label "Terminé !"
    Write-Host ""
    Write-Host ""
    ...
    if ($GenerateHtml -and $results.Count -gt 0) {
        ...
        Write-Host "  Rapport sauvegardé : $reportDir" -ForegroundColor Green
    }
...
while ($true) {
    ...
    switch ($choice) {
        "1" {
            Write-Host ""
            Write-Host "  Démarrage de l'audit complet ($($Global:AuditModules.Count) modules)..." -ForegroundColor Green
            Write-Host ""
        ...
        "2" {
            ...
            if ($null -ne $selectedSet -and $selectedSet.Count -gt 0) {
                Write-Host ""
                Write-Host "  $($selectedSet.Count) module(s) sélectionné(s). Démarrage..." -ForegroundColor Yellow
        ...
        "4" {
            ...
            if ($Global:LastReportPath -and (Test-Path $Global:LastReportPath)) {
                Write-Host ""
                Write-Host "  Ouverture du rapport : $Global:LastReportPath" -ForegroundColor Cyan
                Start-Process $Global:LastReportPath
                Start-Sleep -Seconds 1
            } else {
                Write-Host ""
                Write-Host "  Aucun rapport trouvé. Lancez d'abord un audit (option 1 ou 2)." -ForegroundColor Red
        ...
        "5" {
            Clear-Host
            Show-Banner
            Write-Host "  ---- RÉSUMÉ COMPLET DE LA MACHINE --------------------------" -ForegroundColor Cyan
            Write-Host ""

            # --- SYSTEME ---
            Write-Host ""
            Write-Host "  ---- SYSTÈME --------------------------------------------------------" -ForegroundColor DarkYellow
            Write-Host ""
            try {
                $os      = Get-CimInstance Win32_OperatingSystem
                $cs      = Get-CimInstance Win32_ComputerSystem
                ...
                Write-Host ("    Nom machine    : {0}" -f $cs.Name)                                          -ForegroundColor White
                Write-Host ("    Domaine        : {0}" -f $domaine)                                          -ForegroundColor White
                ...
                Write-Host ("    Durée active  : {0}" -f $uptime)                                           -ForegroundColor White
            } catch {
                Write-Host "    [ERREUR] $($_.Exception.Message)" -ForegroundColor Red
            }
            ...
            # --- DISQUES ---
            Write-Host ""
            Write-Host "  ---- DISQUES -------------------------------------------------------" -ForegroundColor DarkYellow
            Write-Host ""
            try {
                ...
                foreach ($d in $disks) {
                    ...
                    Write-Host ("    {0}  [{1}] {2,3}%  --  {3} Go / {4} Go utilisés" -f $d.DeviceID, $bar, $pct, $used, $total) -ForegroundColor $col
                }
            } catch {
                Write-Host "    [ERREUR] $($_.Exception.Message)" -ForegroundColor Red
            }
            ...
            # --- COMPTES ---
            Write-Host ""
            Write-Host "  ---- COMPTES LOCAUX -------------------------------------------------" -ForegroundColor DarkYellow
            Write-Host ""
            try {
            ...
            # --- CLE WINDOWS ---
            Write-Host ""
            Write-Host "  ---- CLÉ DE LICENCE WINDOWS --------------------------------------" -ForegroundColor DarkYellow
            Write-Host ""
            try {
                $key = (Get-WmiObject -query 'select * from SoftwareLicensingService').OA3xOriginalProductKey
                if ($key) {
                    Write-Host ("    Clé OEM        : {0}" -f $key) -ForegroundColor Green
                } else {
                    ...
                    if ($digitalId) {
                        ...
                        Write-Host ("    Clé produit    : {0}" -f $productKey) -ForegroundColor Green
                    } else {
                        Write-Host "    Clé non trouvée (licence numérique ou indisponible)" -ForegroundColor DarkGray
                    }
                }
            } catch {
                Write-Host "    [ERREUR] $($_.Exception.Message)" -ForegroundColor Red
            ...
            # --- RAPPORT ---
            Write-Host ""
            Write-Host "  ---- RAPPORT -------------------------------------------------------" -ForegroundColor DarkYellow
            Write-Host ""
            if ($Global:LastReportPath -and (Test-Path $Global:LastReportPath)) {
                $rapportDate = (Get-Item $Global:LastReportPath).LastWriteTime.ToString("dd/MM/yyyy HH:mm")
                Write-Host ("    Dernier rapport : {0}" -f $Global:LastReportPath) -ForegroundColor DarkCyan
                Write-Host ("    Généré le       : {0}" -f $rapportDate)            -ForegroundColor DarkCyan
            } else {
                Write-Host "    Aucun rapport HTML généré pour cette machine." -ForegroundColor DarkGray
            }
...
# Identifier les entrées avec MAC dupliqué et isoler la machine concernée' `
    ...
    if (Has "13" "(?i)RDP|(?i)3389|(?i)445|(?i)WinRM|(?i)5985") {
        if (Has "13" "(?i)3389") {
            Add-Action 2 "Pare-feu" "Restreindre l'accès RDP aux IP autorisées uniquement" `
                "Le RDP exposé est le principal vecteur de brute-force et de ransomware." `
                'New-NetFirewallRule -Name "RDP-Restrict" -DisplayName "RDP - Restreindre" -Direction Inbound -Protocol TCP -LocalPort 3389 -RemoteAddress <IP_Admin> -Action Allow
# Bloquer le reste :
netsh advfirewall firewall set rule name="Remote Desktop - User Mode (TCP-In)" new remoteip=<IP_Admin>' `
        ...
        if (Has "13" "(?i)445") {
            Add-Action 1 "Pare-feu" "Bloquer le port SMB (445) en entrée depuis Internet" `
                "Le SMB exposé sur Internet permet l'exploitation de vulnérabilités critiques (EternalBlue, etc.)." `
                'New-NetFirewallRule -Name "Block-SMB-In" -DisplayName "Bloquer SMB entrant" -Direction Inbound -Protocol TCP -LocalPort 445 -Action Block' `
                "CIS 9.2 / ANSSI R17" `
                "statrzo"
        }
    }
