...
Le score n'est pas une simple moyenne. Il est calculé selon une formule inspirée du **CVSS** :

```
Risque(domaine) = Impact × Exploitabilité
  Impact         = poids_domaine / 4        (normalisé 0.25 → 1.0)
  Exploitabilité = (10 - score_domaine) / 10

Score global = 10 - (risque_moyen × 10)
```

Avec deux garde-fous :
- **Plancher punitif** : si un domaine critique (AV, Pare-feu, MAJ, Journaux sécurité) échoue sévèrement, le score global est plafonné — impossible d'obtenir "Bon" avec un antivirus désactivé
- **Malus de couverture** : un audit partiel (peu de modules) tire le score vers 5/10 pour refléter l'incertitude
